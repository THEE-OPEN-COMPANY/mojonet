...
for user_id in range(500):
    ...
    user_dir = ''.join(random.choice(string.ascii_uppercase +
                       string.ascii_lowercase + string.digits) for _ in range(34))
    ...
    for i in range(random.randint(0, 100)):
        data_json["comment_vote"][random.choice(
            comment_uris)] = int(time.time())
    ...
    content_json = {
        "cert_auth_type": "web",
        "cert_user_id": "fakeuser%s@mojoid.bit" % user_id,
    ...
    json.dump(content_json, open("users/%s/content.json" %
              user_dir, "wb"), indent=4)
    print ".",
