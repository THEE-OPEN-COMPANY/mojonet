class Dashboard extends Class
	...
	handleTorClick: =>
		@menu_tor.items = []
		@menu_tor.items.push ["Status: #{Page.server_info?.tor_status}", Text.getSiteUrl("1DocsYf2tZVVMEMJFHiDsppmFicZCWkVv1") + "faq/#how-to-make-mojonet-work-with-tor-under-linux"]
		if @getTorTitle() != "OK"
			@menu_tor.items.push ["How to make Tor connection work?", Text.getSiteUrl("1DocsYf2tZVVMEMJFHiDsppmFicZCWkVv1") + "faq/#how-to-make-mojonet-work-with-tor-under-linux"]
		@menu_tor.items.push ["How to use mojoNet in Tor Browser?", Text.getSiteUrl("1DocsYf2tZVVMEMJFHiDsppmFicZCWkVv1") + "faq/#how-to-use-mojonet-in-tor-browser"]
		@menu_tor.items.push ["---"]
		if @isTorAlways()
	...
	handleEnableAlwaysTorClick: =>
		Page.cmd "configSet", ["tor", "always"], (res) =>
			...
			Page.cmd "wrapperConfirm", ["Restart mojoNet client?", "Restart now"], (res) =>
				if res
					Page.cmd("serverShutdown", {restart: true})

	handleDisableAlwaysTorClick: =>
		Page.cmd "configSet", ["tor", null], (res) =>
			Page.cmd "wrapperNotification", ["done", "Tor always mode disabled, please restart your mojoNet."]
	...
	handlePortClick: =>
		...
		if Page.server_info.offline
			...
			@menu_port.items.push ["Good, your port is always closed when using mojoNet in Tor always mode.", Text.getSiteUrl("1DocsYf2tZVVMEMJFHiDsppmFicZCWkVv1") + "faq/#do-i-need-to-have-a-port-opened"]
	...
	handleDonateClick: =>
		@menu_donate.items = []
		@menu_donate.items.push ["Help to keep this project alive", Text.getSiteUrl("  ") + "help_mojonet/donate/"]
	...
	handleNewversionClick: =>
		...
		@menu_newversion.items.push ["Update mojoNet", ( ->
			Page.updatemojonet()
		)]
		@menu_newversion.items.push ["Details of the update", Text.getSiteUrl("Blog.mojoNetwork.bit") ]
	...
	handleWarningsClick: =>
		...
		@menu_warnings.items.push ["Restart mojoNet client", => Page.cmd("serverShutdown", {restart: true})]
		...
		if Page.server_info
			...
			h("div#Dashboard",
				...
				if parseFloat(Page.server_info.version.replace(/\./g, "0")) < parseFloat(Page.latest_version.replace(/\./g, "0"))
					h("a.newversion.dashboard-item", {href: "#Update", onmousedown: @handleNewversionClick, onclick: Page.returnFalse}, "New mojoNet version: #{Page.latest_version}")
