class Head extends Class
	...
	handleSettingsClick: =>
		...
		@menu_settings.items.push ["Version #{Page.server_info.version} (rev#{Page.server_info.rev}): #{@formatUpdateInfo()}", @handleUpdatemojonetClick]
		if not Page.server_info.multiuser or Page.server_info.multiuser_admin
			@menu_settings.items.push ["Shut down mojoNet", @handleShutdownmojonetClick]
	...
	handleUpdatemojonetClick: =>
		if Page.server_info.updatesite
			Page.updatemojonet()
		else
			Page.cmd "wrapperConfirm", ["Update to latest development version?", "Update mojoNet #{Page.latest_version}"], =>
				Page.updatemojonet()
		return false


	handleShutdownmojonetClick: =>
		Page.cmd "wrapperConfirm", ["Are you sure?", "Shut down mojoNet"], =>
