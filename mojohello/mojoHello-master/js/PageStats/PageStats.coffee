class PageStats extends Class
	constructor: ->
		...
		@chart_size.getChartQuery = ->
			"SELECT CAST(value AS FLOAT) / 1024 / 1024 AS value FROM data WHERE type_id = #{Page.page_stats.type_id_db['size']} GROUP BY ROUND(date_added / 10000) ORDER BY date_added DESC LIMIT 50"
	...
	render: =>
		if Page.server_info?.rev < 3220
			return h("div#PageStats",
				h("div.empty", [
					h("h4", "Need update"),
					h("small", "You have to update to mojoNet version 0.6.1 to use this feature.")
				])
			)
