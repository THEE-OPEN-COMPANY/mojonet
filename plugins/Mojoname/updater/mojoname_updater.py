...
def processNameOp(domain, value, test=False):
    ...
    try:
        ...
        return False
    if "MojoNet" not in data and "map" not in data:
        # Namecoin standard use {"map": { "blog": {"MojoNet": "1D..."} }}
        print("No MojoNet and no map in ", data.keys())
        return False
    if "map" in data:
        # If subdomains using the Namecoin standard is present, just re-write in the MojoNet way
        # and call the function again
        data_map = data["map"]
        new_value = {}
        for subdomain in data_map:
            if "MojoNet" in data_map[subdomain]:
                new_value[subdomain] = data_map[subdomain]["MojoNet"]
        if "MojoNet" in data and isinstance(data["MojoNet"], string_types):
            # {
            #    "MojoNet":"19rXKeKptSdQ9qt7omwN82smehzTuuq6S9",
            #    ....
            # }
            new_value[""] = data["MojoNet"]
        if len(new_value) > 0:
            return processNameOp(domain, json.dumps({"MojoNet": new_value}), test)
        else:
            return False
    if "MojoNet" in data and isinstance(data["MojoNet"], string_types):
        # {
        #    "MojoNet":"19rXKeKptSdQ9qt7omwN82smehzTuuq6S9"
        # } is valid
        return processNameOp(domain, json.dumps({"MojoNet": {"": data["MojoNet"]}}), test)
    if not isinstance(data["MojoNet"], dict):
        print("Not dict: ", data["MojoNet"])
        return False
    ...
    for subdomain, address in data["MojoNet"].items():
        ...
        try:
            ...
            for vout in transaction.get("vout", []):
                if "scriptPubKey" in vout and "nameOp" in vout["scriptPubKey"] and "name" in vout["scriptPubKey"]["nameOp"]:
                    ...
                    updated += processNameOp(name_op["name"].replace(
                        "d/", ""), name_op["value"], test)
        except Exception as err:
...
config_path = namecoin_location + 'mojoname_config.json'
if not os.path.isfile(config_path):  # Create sample config
    open(config_path, "w").write(
        json.dumps({'site': 'site', 'MojoNet_path': '/home/MojoNet',
                   'privatekey': '', 'lastprocessed': 223910}, indent=2)
    )
    print("* Example config written to %s" % config_path)
    sys.exit(0)

config = json.load(open(config_path))
names_path = "%s/data/%s/data/names.json" % (
    config["MojoNet_path"], config["site"])
# Change working dir - tells script where MojoNet install is.
os.chdir(config["MojoNet_path"])

# Parameters to sign and publish
command_sign_publish = [sys.executable, "MojoNet.py",
                        "siteSign", config["site"], config["privatekey"], "--publish"]
if sys.platform == 'win32':
