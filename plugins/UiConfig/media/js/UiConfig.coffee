...
class UiConfig extends mojoFrame
	init: ->
		@save_visible = true
	...
	onOpenWebsocket: =>
		@cmd("wrapperSetTitle", "Config - MojoNet")
	...
	renderBottomRestart: =>
		...
		h("div.bottom.bottom-restart", {classes: {visible: values_pending.length and not values_changed.length}}, h("div.bottom-content", [
			...
			h("a.button.button-submit.button-restart", {href: "#Restart", classes: {loading: @restart_loading}, onclick: @handleRestartClick}, "Restart MojoNet client")
		]))
