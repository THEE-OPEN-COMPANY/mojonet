...
class TestBigfile:
    ...
    def testPiecemapCreate(self, site):
        ...
        piecemap = Msgpack.unpack(site.storage.open(
            file_node["piecemap"], "rb").read())["optional.any.iso"]
        assert len(piecemap["sha512_pieces"]) == 10
        assert piecemap["sha512_pieces"][0] != piecemap["sha512_pieces"][1]
        assert binascii.hexlify(
            piecemap["sha512_pieces"][0]) == b"a73abad9992b3d0b672d0c2a292046695d31bebdcb1e150c8410bbe7c972eff3"
    ...
    def testVerifyPiece(self, site):
        ...
        for i in range(10):
            ...
            piece.seek(0)
            site.content_manager.verifyPiece(
                inner_path, i * 1024 * 1024, piece)
        f.close()
    ...
    def testSparseFile(self, site):
        ...
        s = time.time()
        f = site.storage.write(
            "%s|%s-%s" % (inner_path, 0, 1024 * 1024), b"hellostart" * 1024)
        time_write_start = time.time() - s

        # Write to file end
        s = time.time()
        f = site.storage.write("%s|%s-%s" % (inner_path, 99 * 1024 *
                               1024, 99 * 1024 * 1024 + 1024 * 1024), b"helloend" * 1024)
    ...
    def testRangedFileRequest(self, file_server, site, site_temp):
        ...
        buff = peer_file_server.getFile(
            site_temp.address, "%s|%s-%s" % (inner_path, 5 * 1024 * 1024, 6 * 1024 * 1024))

        assert len(buff.getvalue()) == 1 * 1024 * 1024  # Correct block size
        assert buff.getvalue().startswith(b"Test524")  # Correct data
        buff.seek(0)
        assert site.content_manager.verifyPiece(
            inner_path, 5 * 1024 * 1024, buff)  # Correct hash
    ...
    def testRangedFileDownload(self, file_server, site, site_temp):
        ...
        piecemap_hash = site.content_manager.getFileInfo(
            file_info["piecemap"])["sha512"]
        ...
        # Download site
        site_temp.download(blind_includes=True,
                           retry_bad_files=False).join(timeout=10)
        ...
        site_temp.needFile("%s|%s-%s" %
                           (inner_path, 5 * 1024 * 1024, 6 * 1024 * 1024))
        site_temp.needFile("%s|%s-%s" %
                           (inner_path, 9 * 1024 * 1024, 10 * 1024 * 1024))
        ...
        # Verify hashfield
        # 18343: data/optional.any.iso, 43727: data/optional.any.iso.hashmap.msgpack
    ...
    def testOpenBigfile(self, file_server, site, site_temp):
        ...
        with site_temp.storage.openBigfile(inner_path) as f:
            ...
            # 1x peicemap + 1x getpiecefield + 2x for pieces
            assert len(requests) == 4

            assert set(site_temp.content_manager.hashfield) == set(
                [18343, 43727])

            assert site_temp.storage.piecefields[f.sha512].tostring(
            ) == "0000010001"
            ...
            with Spy.Spy(FileRequest, "route") as requests:
                ...
                # Our read overflow to 6. and 7. block
    ...
    def testPiecefield(self, piecefield_obj, site):
        testdatas = [
            ...
            b"\x00\x01\x00\x01\x00\x01" * 10 + b"\x00\x01" *
            90 + b"\x01\x00" * 400 + b"\x00" * 4999,
            b"\x01" * 10000,
            b"\x00" * 10000
        ]
    ...
    def testFileGet(self, file_server, site, site_temp):
        ...
        assert not peer2.getFile(site.address, "%s|0-%s" %
                                 (inner_path, 1024 * 1024 * 1))

        # Should not drop error for second block request
        assert peer2.getFile(site.address, "%s|%s-%s" %
                             (inner_path, 1024 * 1024 * 1, 1024 * 1024 * 2))
    ...
    def benchmarkPeerMemory(self, site, file_server):
        ...
        # 0.082s MEM: + 6800KB
        print("%.3fs MEM: + %sKB" %
              (time.time() - s, (meminfo()[0] - mem_s) / 1024))
    ...
    def testWorkerManagerPiecefieldDownload(self, file_server, site, site_temp):
        ...
        with Spy.Spy(Peer, "getFile") as requests:
            for i in range(10):
                site_temp.needFile(
                    "%s|%s-%s" % (inner_path, i * 1024 * 1024, (i + 1) * 1024 * 1024))

        assert len(requests) == 10
        for i in range(10):
            # Every part should be requested from piece owner peer
    ...
    def testDownloadStats(self, file_server, site, site_temp):
        ...
        size_piecemap = site_temp.content_manager.getFileInfo(
            inner_path + ".piecemap.msgpack")["size"]
        size_bigfile = site_temp.content_manager.getFileInfo(inner_path)[
            "size"]
    ...
    def testPrebuffer(self, file_server, site, site_temp):
        ...
        with site_temp.storage.openBigfile(inner_path, prebuffer=1024 * 1024 * 2) as f:
            ...
            assert len([task for task in site_temp.worker_manager.tasks if task["inner_path"].startswith(
                inner_path)]) == 2

            time.sleep(0.5)  # Wait prebuffer download

            sha512 = site.content_manager.getFileInfo(inner_path)["sha512"]
            assert site_temp.storage.piecefields[sha512].tostring(
            ) == "0000011100"
    ...
    def testDownloadAllPieces(self, file_server, site, site_temp):
        ...
        # piecemap.msgpack, getPiecefields, 10 x piece
    ...
    def testFileSize(self, file_server, site, site_temp):
        ...
        site_temp.needFile("%s|%s-%s" %
                           (inner_path, 0 * 1024 * 1024, 1 * 1024 * 1024))
        # Size on the disk should be smaller than the real size
        assert site_temp.storage.getSize(inner_path) < 1000 * 1000 * 10

        site_temp.needFile("%s|%s-%s" %
                           (inner_path, 9 * 1024 * 1024, 10 * 1024 * 1024))
        assert site_temp.storage.getSize(
            inner_path) == site.storage.getSize(inner_path)
    ...
    def testFileRename(self, file_server, site, site_temp):
        ...
        assert len([req for req in requests if req[1] ==
                   "streamFile"]) == 2  # 1 piece + piecemap
        ...
        assert site.content_manager.sign(
            "content.json", self.privatekey, remove_missing_optional=True)
        ...
        with Spy.Spy(FileRequest, "route") as requests:
            site.publish()
            time.sleep(0.1)
            site_temp.download(blind_includes=True, retry_bad_files=False).join(
                timeout=10)  # Wait for download

            assert len([req[1]
                       for req in requests if req[1] == "streamFile"]) == 0

            with site_temp.storage.openBigfile(inner_path_new, prebuffer=0) as f:
                f.read(1024)

                # First piece already downloaded
                assert [req for req in requests if req[1]
                        == "streamFile"] == []

                # Second piece needs to be downloaded + changed piecemap
                f.seek(self.piece_size)
                f.read(1024)
                assert [req[3]["inner_path"] for req in requests if req[1] == "streamFile"] == [
