...
class UiRequestPlugin(object):
    def isCorsAllowed(self, path):
        if path == "/MojoNet-Internal/BigfileUpload":
            return True
        else:
    ...
    def handleBigfileUpload(self, upload_info, read):
        ...
        if len(piecemap_info["sha512_pieces"]) == 1:  # Small file, don't split
            ...
            piecemap_relative_path = upload_info["piecemap"][len(
...
class UiWebsocketPlugin(object):
    def actionBigfileUploadInit(self, to, inner_path, size, protocol="xhr"):
        ...
        if protocol == "xhr":
            return {
                "url": "/MojoNet-Internal/BigfileUpload?upload_nonce=" + nonce,
                "piece_size": piece_size,
                "inner_path": inner_path,
            ...
            if server_url:
                ...
                origin = "{origin}"
            return {
                "url": origin + "/MojoNet-Internal/BigfileUploadWebsocket?upload_nonce=" + nonce,
                "piece_size": piece_size,
                "inner_path": inner_path,
                "file_relative_path": file_relative_path
            }
            ...
            if file_info:
                ...
                if relative_path in content_json.get("files_optional", {}):
                    ...
                    try:
                        ...
                        self.log.error("File %s delete error: %s" %
...
class ContentManagerPlugin(object):
    ...
    def readFile(self, read_func, size, buff_size=1024 * 64):
        ...
        while 1:
            ...
            if part_num % 100 == 0:  # Avoid blocking MojoNet execution during upload
                time.sleep(0.001)
                ...
                if piece_recv >= piece_size:
                    ...
                    if len(piece_hashes) % 100 == 0 or recv == size:
                        self.log.info("- [HASHING:%.0f%%] Pieces: %s, %.1fMB/%.1fMB" % (
                            float(recv) / size * 100, len(piece_hashes), recv /
                            1024 / 1024, size / 1024 / 1024
                        ))
                        part = ""
            if len(part) > 0:
                piece_digest = piece_hash.digest()
    ...
    def hashFile(self, dir_inner_path, file_relative_path, optional=False):
        ...
        if not hash or not piecemap_relative_path:  # Not in content.json yet
            ...
            if not piecemap_relative_path:
                ...
                back.update(super(ContentManagerPlugin, self).hashFile(
        ...
        back[file_relative_path] = {"sha512": hash, "size": file_size,
                                    "piecemap": piecemap_relative_path, "piece_size": piece_size}
        return back
    ...
    def getPiecemap(self, inner_path):
        ...
        piecemap = Msgpack.unpack(self.site.storage.open(
            piecemap_inner_path, "rb").read())[helper.getFilename(inner_path)]
    ...
    def verifyPiece(self, inner_path, pos, piece):
        try:
            ...
            raise VerifyError("Unable to download piecemap: %s" %
...
class SiteStoragePlugin(object):
    ...
    def createSparseFile(self, inner_path, size, sha512=None):
        ...
        if sha512 and sha512 in self.piecefields:
            self.log.debug(
                "%s: File not exists, but has piecefield. Deleting piecefield." % inner_path)
        ...
        if os.path.isfile(file_path):
            if sha512 not in self.piecefields:
                if open(file_path, "rb").read(128) == b"\0" * 128:
                    ...
                    piece_data = b"\x01"
                self.log.debug("%s: File exists, but not in piecefield. Filling piecefiled with %s * %s." %
        ...
        with self.read_lock:
            ...
            while 1:
                ...
                if not self.piecefield[piece_i]:
                    requests.append(self.site.needFile(
                        "%s|%s-%s" % (self.inner_path, pos_from, pos_to), blocking=False, update=True, priority=10))
                pos += self.piece_size
            ...
            if self.prebuffer:
                ...
                while 1:
                    ...
                    pos_to = pos_from + self.piece_size
                    if not self.piecefield[piece_i]:
                        self.site.needFile("%s|%s-%s" % (self.inner_path, pos_from, pos_to),
                                           blocking=False, update=True, priority=max(0, priority))
                    priority -= 1
                    pos += self.piece_size
...
class FileRequestPlugin(object):
    ...
    def actionGetPiecefields(self, params):
        ...
        peer = site.addPeer(self.connection.ip,
                            self.connection.port, return_peer=True)
        if not peer.connection:  # Just added
            peer.connect(self.connection)  # Assign current connection to peer

        piecefields_packed = {sha512: piecefield.pack(
    ...
    def actionSetPiecefields(self, params):
        ...
        # Add or get peer
        peer = site.addPeer(self.connection.ip, self.connection.port,
                            return_peer=True, connection=self.connection)
...
class PeerPlugin(object):
    ...
    def updatePiecefields(self, force=False):
        ...
        try:
            ...
            self.log("Invalid updatePiecefields response: %s" %
                     Debug.formatException(err))
...
class SitePlugin(object):
    ...
    def getSettingsCache(self):
        ...
        if self.storage.piecefields:
            back["piecefields"] = {sha512: base64.b64encode(piecefield.pack()).decode(
                "utf8") for sha512, piecefield in self.storage.piecefields.items()}
        return back
        ...
        if inner_path.endswith("|all"):
            ...
            for piece_i in range(piece_num):
                ...
                if not piecefield or not piecefield[piece_i]:
                    inner_path_piece = "%s|%s-%s" % (inner_path,
                                                     piece_from, piece_to)
...
class ConfigPlugin(object):
    def createArguments(self):
        ...
        group.add_argument('--autodownload_bigfile_size_limit',
                           help='Also download bigfiles smaller than this limit if help distribute option is checked', default=10, metavar="MB", type=int)
        group.add_argument('--bigfile_size_limit', help='Maximum size of downloaded big files',
                           default=False, metavar="MB", type=int)
