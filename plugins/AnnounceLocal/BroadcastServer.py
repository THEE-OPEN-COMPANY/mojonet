...
class BroadcastServer(object):
            ...
            try:
                ...
                self.log.error(
                    ...
                    (self.listen_ip, self.listen_port,
                     Debug.formatException(err), retry)
                )
                time.sleep(retry)
    ...
    def start(self):  # Listens for discover requests
        ...
        while self.running:
            ...
            try:
                ...
                self.log.error("Handlemessage error: %s" %
    ...
    def broadcast(self, message, port=None):
        ...
        self.log.debug("Broadcast using ips %s on port %s: %s" %
    ...
    def handleMessage(self, addr, message):
        ...
        if sender["service"] != "MojoNet" or sender["peer_id"] == self.sender_info["peer_id"]:
