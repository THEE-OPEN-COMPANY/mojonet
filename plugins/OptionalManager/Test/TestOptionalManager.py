...
class TestOptionalManager:
    def testDbFill(self, site):
        ...
        assert contents.db.execute("SELECT COUNT(*) FROM file_optional WHERE is_downloaded = 1").fetchone()[
            0] == len(site.content_manager.hashfield)
    ...
    def testSetContent(self, site):
        ...
        num_optional_files_before = contents.db.execute(
            "SELECT COUNT(*) FROM file_optional").fetchone()[0]
        contents["content.json"] = new_content
        assert contents.db.execute(
            "SELECT COUNT(*) FROM file_optional").fetchone()[0] > num_optional_files_before
    ...
    def testVerifyFiles(self, site):
        ...
        contents["content.json"] = new_content
        file_row = contents.db.execute(
            "SELECT * FROM file_optional WHERE inner_path = 'testfile'").fetchone()
        assert not file_row["is_downloaded"]

        # Write file from outside of MojoNet
        # For quick check hash does not matter only file size
        ...
        # Delete file outside of MojoNet
    ...
    def testVerifyFilesSameHashId(self, site):
        ...
        assert site.content_manager.hashfield.getHashId(
            "aaaabbbbcccc") == site.content_manager.hashfield.getHashId("aaaabbbbdddd")

        # Write files from outside of MojoNet (For quick check hash does not matter only file size)
        ...
        assert site.content_manager.hashfield.getHashId(
            "aaaabbbbcccc") in site.content_manager.hashfield
        ...
        assert site.content_manager.isDownloaded("testfile2")
        assert site.content_manager.hashfield.getHashId(
            "aaaabbbbdddd") in site.content_manager.hashfield

    def testIsPinned(self, site):
        assert not site.content_manager.isPinned(
            "data/img/mojotalk-upvote.png")
        site.content_manager.setPin("data/img/mojotalk-upvote.png", True)
        assert site.content_manager.isPinned("data/img/mojotalk-upvote.png")
    ...
    def testOptionalDelete(self, site):
        ...
        site.content_manager.setPin("data/img/mojotalk-upvote.png", True)
        site.content_manager.setPin("data/img/mojoid.png", False)
        new_content = copy.deepcopy(contents["content.json"])
        del new_content["files_optional"]["data/img/mojotalk-upvote.png"]
        del new_content["files_optional"]["data/img/mojoid.png"]

        assert site.storage.isFile("data/img/mojotalk-upvote.png")
        assert site.storage.isFile("data/img/mojoid.png")

        site.storage.writeJson("content.json", new_content)
        site.content_manager.loadContent("content.json", force=True)

        assert not site.storage.isFile("data/img/mojoid.png")
        assert site.storage.isFile("data/img/mojotalk-upvote.png")
    ...
    def testOptionalRename(self, site):
        ...
        site.content_manager.setPin("data/img/mojotalk-upvote.png", True)
        new_content = copy.deepcopy(contents["content.json"])
        new_content["files_optional"]["data/img/mojotalk-upvote-new.png"] = new_content["files_optional"]["data/img/mojotalk-upvote.png"]
        del new_content["files_optional"]["data/img/mojotalk-upvote.png"]
        ...
        assert not site.storage.isFile("data/img/mojotalk-upvote.png")
        assert not site.content_manager.isPinned(
            "data/img/mojotalk-upvote.png")
        assert site.content_manager.isPinned(
            "data/img/mojotalk-upvote-new.png")
        assert site.storage.isFile("data/img/mojotalk-upvote-new.png")
