...
class TrackerStorage(object):
    ...
    def onTrackerFound(self, tracker_address, type="shared", my=False):
        if not tracker_address.startswith("mojo://"):
            return False
    ...
    def onTrackerError(self, tracker_address):
        ...
        if trackers[tracker_address]["num_error"] > error_limit and trackers[tracker_address]["time_success"] < time.time() - 60 * 60:
            self.log.debug("Tracker %s looks down, removing." %
                           tracker_address)
    ...
    def load(self):
        ...
        for address, tracker in list(trackers.items()):
            tracker["num_error"] = 0
            if not address.startswith("mojo://"):
    ...
    def discoverTrackers(self, peers):
        ...
        self.log.debug("Trackers discovered from %s/%s peers in %.3fs" %
                       (num_success, len(peers), time.time() - s))
...
class SiteAnnouncerPlugin(object):
    def getTrackers(self):
        if tracker_storage.time_discover < time.time() - 5 * 60:
            ...
            gevent.spawn(tracker_storage.discoverTrackers,
                         self.site.getConnectedPeers())
...
class FileRequestPlugin(object):
    def actionGetTrackers(self, params):
        shared_trackers = list(
            tracker_storage.getWorkingTrackers("shared").keys())
    ...
    def portCheck(self, *args, **kwargs):
        ...
        if res and not config.tor == "always" and "Bootstrapper" in PluginManager.plugin_manager.plugin_names:
            for ip in self.ip_external_list:
                my_tracker_address = "mojo://%s:%s" % (
                    ip, config.fileserver_port)
...
class ConfigPlugin(object):
    def createArguments(self):
        ...
        group.add_argument('--working_shared_trackers_limit',
                           help='Stop discovering new shared trackers after this number of shared trackers reached', default=5, type=int, metavar='limit')
