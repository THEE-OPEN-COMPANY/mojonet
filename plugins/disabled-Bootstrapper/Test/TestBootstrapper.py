...
class TestBootstrapper:
    ...
    def testBootstrapperDb(self, file_server, bootstrapper_db):
        ...
        bootstrapper_db.peerAnnounce(ip_type, file_server.ip_external, port=15441, hashes=[
                                     hash1, hash2], delete_missing_hashes=True)
        ...
        bootstrapper_db.peerAnnounce(ip_type, file_server.ip_external, port=15441, hashes=[
                                     hash1], delete_missing_hashes=True)
        res = peer.request("announce", {
            "hashes": [hash1, hash2],
            "port": 15441, "need_types": [ip_type], "need_num": 10, "add": [ip_type]
        })
        ...
        bootstrapper_db.peerAnnounce(ip_type, file_server.ip_external, port=15441, hashes=[
                                     hash1, hash2, hash3], delete_missing_hashes=True)
        res = peer.request("announce", {
        ...
        # Test DB cleanup
        assert [row[0] for row in bootstrapper_db.execute("SELECT address FROM peer").fetchall()] == [
            file_server.ip_external]  # 127.0.0.1 never get added to db

        # Delete peers
        bootstrapper_db.execute("DELETE FROM peer WHERE address = ?", [
                                file_server.ip_external])
        assert bootstrapper_db.execute(
            "SELECT COUNT(*) AS num FROM peer_to_hash").fetchone()["num"] == 0

        assert bootstrapper_db.execute(
            "SELECT COUNT(*) AS num FROM hash").fetchone()["num"] == 3  # 3 sites
        assert bootstrapper_db.execute(
            "SELECT COUNT(*) AS num FROM peer").fetchone()["num"] == 0  # 0 peer
    ...
    def testPassive(self, file_server, bootstrapper_db):
        ...
        bootstrapper_db.peerAnnounce(
            ip_type, address=None, port=15441, hashes=[hash1])
        res = peer.request("announce", {
    ...
    def testAddOnion(self, file_server, site, bootstrapper_db, tor_manager):
        ...
        bootstrapper_db.peerAnnounce(
            ip_type="ipv4", address="1.2.3.4", port=1234, hashes=[hash1, hash2, hash3])
        res = peer.request("announce", {
        ...
        site_peers = bootstrapper_db.peerList(
            address="1.2.3.4", port=1234, hash=hash1)
        assert len(site_peers["onion"]) == 0
        assert "onion_sign_this" in res

        # Sign the nonces
        sign1 = CryptRsa.sign(
            res["onion_sign_this"].encode(), tor_manager.getPrivatekey(onion1))
        sign2 = CryptRsa.sign(
            res["onion_sign_this"].encode(), tor_manager.getPrivatekey(onion2))
        ...
        assert "onion_sign_this" in res
        site_peers1 = bootstrapper_db.peerList(
            address="1.2.3.4", port=1234, hash=hash1)
        ...
        site_peers2 = bootstrapper_db.peerList(
            address="1.2.3.4", port=1234, hash=hash2)
        assert len(site_peers2["onion"]) == 1
        site_peers3 = bootstrapper_db.peerList(
            address="1.2.3.4", port=1234, hash=hash3)
        ...
        assert helper.unpackOnionAddress(site_peers1["onion"][0])[
            0] == onion1 + ".onion"
        assert helper.unpackOnionAddress(site_peers2["onion"][0])[
            0] == onion1 + ".onion"
        assert helper.unpackOnionAddress(site_peers3["onion"][0])[
            0] == onion2 + ".onion"
    ...
    def testRequestPeers(self, file_server, site, bootstrapper_db, tor_manager):
        ...
        bootstrapper_db.peerAnnounce(
            ip_type="ipv4", address="1.2.3.4", port=1234, hashes=[hash])
        site.announcer.announceTracker(
            "mojo://%s:%s" % (file_server.ip, file_server.port))
        assert len(site.peers) == 1

        # Test onion address store
        bootstrapper_db.peerAnnounce(
            ip_type="onion", address="bka4ht2bzxchy44r", port=1234, hashes=[hash], onion_signed=True)
        site.announcer.announceTracker(
            "mojo://%s:%s" % (file_server.ip, file_server.port))
    ...
    def testBackwardCompatibility(self, file_server, bootstrapper_db):
        ...
        bootstrapper_db.peerAnnounce("ipv4", file_server.ip_external, port=15441, hashes=[
                                     hash1], delete_missing_hashes=True)
