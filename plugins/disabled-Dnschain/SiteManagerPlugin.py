...
class SiteManagerPlugin(object):
    ...
    def resolveDomainDnschainNet(self, domain):
        try:
            ...
            with gevent.Timeout(5, Exception("Timeout: 5s")):
                res = Http.get(
                    "https://api.dnschain.net/v1/namecoin/key/%s" % top_domain).read()
                data = json.loads(res)["data"]["value"]
                if "MojoNet" in data:
                    for key, val in data["MojoNet"].items():
                        self.dns_cache[key+"."+top_domain] = [val,
                                                              time.time()+60*60*5]  # Cache for 5 hours
                    self.saveDnsCache()
                    return data["MojoNet"].get(sub_domain)
            # Not found
            return address
    ...
    def resolveDomainDnschainInfo(self, domain):
        try:
            ...
            address = None
            with gevent.Timeout(5, Exception("Timeout: 5s")):
                res = Http.get("https://dnschain.info/bit/d/%s" %
                               re.sub(r"\.bit$", "", top_domain)).read()
            ...
            log.debug("Dnschain.info %s resolve error: %s" %
                      (domain, Debug.formatException(err)))
    ...
    def resolveDomain(self, domain):
        ...
        # Found in cache and its not expired
        if domain_details and time.time() < domain_details[1]:
            ...
            if thread_dnschain_info.value and thread_dnschain_net.value:  # Booth successfull
                if thread_dnschain_info.value == thread_dnschain_net.value:  # Same returned value
                    ...
                    log.error("Dns %s missmatch: %s != %s" % (
                        domain, thread_dnschain_info.value, thread_dnschain_net.value))

            # Problem during resolve
            if domain_details:  # Resolve failed, but we have it in the cache
                # Dont try again for 1 hour
                domain_details[1] = time.time()+60*60
                return domain_details[0]
            else:  # Not found in cache
                # Don't check again for 1 min
