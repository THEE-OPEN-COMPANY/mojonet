...
(function () {
  ...
  PluginList = (function (superClass) {
    ...
    PluginList.prototype.savePluginStatus = function (plugin, is_enabled) {
      Page.cmd(
        "pluginConfigSet",
        [plugin.source, plugin.inner_path, "enabled", is_enabled],
        (function (_this) {
          return function (res) {
            if (res === "ok") {
    ...
    PluginList.prototype.handleUpdateClick = function (e) {
      ...
      Page.cmd(
        "pluginUpdate",
        [plugin.source, plugin.inner_path],
        (function (_this) {
          return function (res) {
            if (res === "ok") {
              Page.cmd("wrapperNotification", [
                "done",
                "Plugin " + plugin.name + " updated to latest version",
              ]);
              Page.updatePlugins();
            } else {
    ...
    PluginList.prototype.handleDeleteClick = function (e) {
      ...
      if (plugin.loaded) {
        Page.cmd("wrapperNotification", [
          "info",
          "You can only delete plugin that are not currently active",
        ]);
        return false;
      }
      node.classList.add("loading");
      Page.cmd(
        "wrapperConfirm",
        ["Delete " + plugin.name + " plugin?", "Delete"],
        (function (_this) {
          return function (res) {
            if (!res) {
            ...
            return function (plugin) {
              ...
              if (plugin.source !== "builtin") {
                ...
                if ((ref = plugin.site_info) != null ? ref.rev : void 0) {
                  ...
                  tag_update = h(
                    "span.version-missing",
                    "(unable to get latest vesion: update site missing)"
                  );
                }
                ...
                tag_source = h("div.source", [
                  ...
                  h(
                    ...
                    plugin.site_title ? plugin.site_title : plugin.source
                  ),
                  " /" + plugin.inner_path,
                ]);
                tag_delete = h(
              ...
              if (plugin.enabled !== plugin.loaded || plugin.updated) {
                ...
                marker_title =
                  "Changed from default status (click to reset to " +
                  plugin.info["default"] +
                  ")";
                is_pending = false;
              }
              is_changed =
                plugin.enabled !== enabled_default &&
                plugin.owner === "builtin";
              return h(
                "div.plugin",
                {
                  key: plugin.name,
...
(function () {
  ...
  UiPluginManager = (function (superClass) {
    ...
    UiPluginManager.prototype.onOpenWebsocket = function () {
      this.cmd("wrapperSetTitle", "Plugin manager - MojoNet");
      this.cmd(
        "serverInfo",
        {},
        (function (_this) {
      ...
      return this.cmd(
        ...
        (function (_this) {
          return function (res) {
            ...
            _this.plugin_list_builtin.plugins = plugins_builtin.sort(function (
              a,
              b
            ) {
            ...
            _this.plugin_list_custom.plugins = plugins_custom.sort(function (
              a,
              b
            ) {
              return a.name.localeCompare(b.name);
    ...
    UiPluginManager.prototype.render = function () {
      ...
      return h("div.content", [
        h("div.section", [
          (
            (ref = this.plugin_list_custom.plugins) != null
              ? ref.length
              : void 0
          )
            ? [
                h("h2", "Installed third-party plugins"),
                this.plugin_list_custom.render(),
              ]
            : void 0,
          h("h2", "Built-in plugins"),
          this.plugin_list_builtin.render(),
        ]),
      ]);
    };
