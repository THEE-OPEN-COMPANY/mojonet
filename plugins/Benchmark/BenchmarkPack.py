...
class ActionsPlugin:
            ...
            with zipfile.ZipFile(zip_path) as archive:
                for f in archive.filelist:
                    assert f.filename.startswith(
                        file_name), "Invalid filename: %s != %s" % (f.filename, file_name)
                    data = archive.open(f.filename).read()
                    assert archive.open(f.filename).read(
                    ) == test_data, "Invalid data: %s..." % data[0:30]
            yield "."
    ...
    def testPackMsgpack(self, num_run=1):
        ...
        data = OrderedDict(
            sorted({"int": 1024 * 1024 * 1024, "float": 12345.67890,
                   "text": "hello" * 1024, "binary": binary}.items())
        )
        ...
        assert data_packed == data_packed_valid, "%s<br>!=<br>%s" % (
            repr(data_packed), repr(data_packed_valid))
