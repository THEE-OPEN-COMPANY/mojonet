...
class SiteManagerPlugin(object):
    def load(self, *args, **kwargs):
        ...
        self.log = logging.getLogger("MojoNetLocal Plugin")
        ...
        if not config.namecoin_host or not config.namecoin_rpcport or not config.namecoin_rpcuser or not config.namecoin_rpcpassword:
            ...
            self.log.error(
                "Missing parameters to connect to namecoin node. Please check all the arguments needed with '--help'. MojoNet will continue working without it.")
            return
        ...
        try:
            ...
            self.log.error(
                "The Namecoin node is unreachable. Please check the configuration value are correct. MojoNet will continue working without it.")
    ...
    def resolveDomain(self, domain):
        ...
        if "MojoNet" in domain_object["value"]:
            MojoNet_domains = json.loads(domain_object["value"])["MojoNet"]

            if isinstance(MojoNet_domains, str):
                # {
                #    "MojoNet":"19rXKeKptSdQ9qt7omwN82smehzTuuq6S9"
                # } is valid
                MojoNet_domains = {"": MojoNet_domains}

            self.cache[domain] = {
                "addresses_resolved": MojoNet_domains, "time": time.time()}

        elif "map" in domain_object["value"]:
            # Namecoin standard use {"map": { "blog": {"MojoNet": "1D..."} }}
            data_map = json.loads(domain_object["value"])["map"]

            MojoNet_domains = dict()
            for subdomain in data_map:
                if "MojoNet" in data_map[subdomain]:
                    MojoNet_domains[subdomain] = data_map[subdomain]["MojoNet"]
            if "MojoNet" in data_map and isinstance(data_map["MojoNet"], str):
                # {"map":{
                #    "MojoNet":"19rXKeKptSdQ9qt7omwN82smehzTuuq6S9",
                # }}
                MojoNet_domains[""] = data_map["MojoNet"]

            self.cache[domain] = {
                "addresses_resolved": MojoNet_domains, "time": time.time()}

        else:
            # No MojoNet address registered
            return None
...
class ConfigPlugin(object):
    def createArguments(self):
        group = self.parser.add_argument_group("mojoname Local plugin")
