        ...
        if not back_generator:  # Wrapper error or not string returned, injection not possible
            ...
            inject_html = """
                ...
                <script nonce="{script_nonce}">
                 setTimeout(function() {
                    mojoframe.cmd("wrapperNotification", ["done", "{message}<br><small>You have been logged in successfully</small>", 5000])
                 }, 1000)
                </script>
                </body>
                </html>
...
class UiWebsocketPlugin(object):
    ...
    def actionUserLogout(self, to):
        ...
        script += "$('#button_notification').on('click', function() { mojoframe.cmd(\"userLoginForm\", []); });"
    ...
    def actionUserSet(self, to, master_address):
        ...
        script += "mojoframe.cmd('wrapperReload', ['login=done']);"
    ...
    def actionUserSelectForm(self, to):
        ...
        for master_address, user in user_manager.list().items():
            ...
            if user.certs:
                ...
                title = "%s@%s" % (user.certs[first_cert]
                                   ["auth_user_name"], first_cert)
            else:
    ...
    def actionCertAdd(self, *args, **kwargs):
        ...
        message = """
            ...
            <b>Hello, welcome to mojoProxy!</b><div style='margin-top: 8px'>A new, unique account created for you:</div>
            ...
            <div style='text-align: center; font-size: 85%; margin-bottom: 10px;'>
             ...
             class='masterseed_download' download='MojoNet_private_key.backup'>Download backup as text file</a>
            </div>
            <div>
        ...
        script = """
            ...
            $("#button_notification_download").on("mousedown", function() {
                this.href = window.URL.createObjectURL(new Blob(["MojoNet user master seed:\\r\\n{master_seed}"]))
            })
