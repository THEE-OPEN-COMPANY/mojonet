...
class SiteAnnouncerPlugin(object):
    def getTrackerHandler(self, protocol):
        if protocol == "mojo":
            return self.announceTrackermojo
        else:
            return super(SiteAnnouncerPlugin, self).getTrackerHandler(protocol)

    def announceTrackermojo(self, tracker_address, mode="start", num_want=10):
        ...
        self.site.log.debug(
            "Tracker announce result: mojo://%s (sites: %s, new peers: %s, add: %s, mode: %s) in %.3fs" %
            (tracker_address, site_index, peers_added,
             add_types, mode, time.time() - s)
        )
