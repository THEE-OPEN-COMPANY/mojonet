...
class ActionsPlugin(object):
    ...
    def getAutorunPath(self):
        return "%s\\MojoNet.cmd" % winfolders.get(winfolders.STARTUP)
    ...
    def formatAutorun(self):
        ...
        if sys.platform == 'win32':
            args = ['"%s"' %
                    arg for arg in args if arg and arg not in ignored_args]
        cmd = " ".join(args)

        # Dont open browser on autorun
        cmd = cmd.replace("start.py", "MojoNet.py").strip()
        cmd += ' --open_browser ""'
    ...
    def titleAutorun(self):
        translate = _["Start MojoNet when Windows starts"]
