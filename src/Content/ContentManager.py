...
class ContentManager(object):
    ...
    def loadContents(self):
        ...
        if "hashfield" in self.site.settings.get("cache", {}):
            self.hashfield.frombytes(base64.b64decode(
                self.site.settings["cache"]["hashfield"]))
    ...
    def getFileChanges(self, old_files, new_files):
        deleted = {key: val for key, val in old_files.items()
                   if key not in new_files}
        deleted_hashes = {val.get("sha512"): key for key,
                          val in old_files.items() if key not in new_files}
        added = {key: val for key, val in new_files.items()
    ...
    def loadContent(self, content_inner_path="content.json", add_bad_files=True, delete_removed_files=True, load_includes=True, force=False):
                ...
                if not force and old_content and not self.site.settings.get("own"):
                    for line in open(content_path):
                        ...
                        if match and float(match.group(1)) <= old_content.get("modified", 0):
                            self.log.debug(
                                "%s loadContent same json file, skipping" % content_inner_path)
                            return [], []
        ...
        try:
            ...
            for relative_path, info in new_content.get("files_optional", {}).items():
                ...
                if old_content and old_content.get("files_optional", {}).get(relative_path):
                    # We have the file in the old content
                    old_hash = old_content["files_optional"][relative_path].get(
                        "sha512")
                    if old_hash != new_hash and self.site.isDownloadable(file_inner_path):
                        ...
                        try:
                            ...
                            self.optionalRemoved(
                                file_inner_path, old_hash_id, old_content["files_optional"][relative_path]["size"])
                            self.optionalDelete(file_inner_path)
                            self.log.debug(
                                "Deleted changed optional file: %s" % file_inner_path)
                        except Exception as err:
                            self.log.warning("Error deleting file %s: %s" % (
            ...
            if old_content:
                ...
                for relative_path_old, relative_path_new in renamed.items():
                    ...
                    if self.site.storage.isFile(relative_path_old):
                        try:
                            ...
                            self.log.debug("Renamed: %s -> %s" %
                                           (relative_path_old, relative_path_new))
                        except Exception as err:
                            self.log.warning(
                                "Error renaming file: %s -> %s %s" % (relative_path_old, relative_path_new, err))
                ...
                if deleted and not self.site.settings.get("own"):
                    ...
                    for file_relative_path in deleted:
                        ...
                        try:
                            ...
                            if old_content.get("files_optional") and old_content["files_optional"].get(file_relative_path):
                                self.optionalDelete(file_inner_path)
                                old_hash = old_content["files_optional"][file_relative_path].get(
                                    "sha512")
                                if self.hashfield.hasHash(old_hash):
                                    old_hash_id = self.hashfield.getHashId(
                                        old_hash)
                                    self.optionalRemoved(
                                        file_inner_path, old_hash_id, old_content["files_optional"][file_relative_path]["size"])
                            else:
                                self.site.storage.delete(file_inner_path)

                            self.log.debug("Deleted file: %s" %
                                           file_inner_path)
                        except Exception as err:
                    ...
                    # Cleanup empty dirs
                    tree = {root: [dirs, files] for root, dirs, files in os.walk(
                    ...
                    for root in sorted(tree, key=len, reverse=True):
                        ...
                        if dirs == [] and files == []:
                            root_inner_path = self.site.storage.getInnerPath(
                                root.replace("\\", "/"))
                            self.log.debug(
                                "Empty directory: %s, cleaning up." % root_inner_path)
                            try:
                                ...
                                self.log.debug("Error deleting empty directory %s: %s" % (
                                    root_inner_path, err))

            # Check archived
            if old_content and "user_contents" in new_content and "archived" in new_content["user_contents"]:
                old_archived = old_content.get(
                    "user_contents", {}).get("archived", {})
                new_archived = new_content.get(
                    "user_contents", {}).get("archived", {})
                self.log.debug("old archived: %s, new archived: %s" %
            ...
            if old_content and "user_contents" in new_content and "archived_before" in new_content["user_contents"]:
                old_archived_before = old_content.get(
                    "user_contents", {}).get("archived_before", 0)
                new_archived_before = new_content.get(
                    "user_contents", {}).get("archived_before", 0)
                if old_archived_before != new_archived_before:
                    self.log.debug("Archived before changed: %s -> %s" %
                    ...
                    self.log.debug("Archived removed contents: %s, removed bad files: %s" % (
            ...
            self.log.warning("%s parse error: %s" %
        ...
        if add_bad_files:
            for inner_path in changed:
                self.site.bad_files[inner_path] = self.site.bad_files.get(
                    inner_path, 0) + 1
    ...
    def isArchived(self, inner_path, modified):
        ...
        if file_info:
            ...
            time_directory_archived = file_info.get(
                "archived", {}).get(relative_directory, 0)
        ...
        if inner_path.endswith("content.json"):
            try:
                is_valid = self.verifyFile(
                    inner_path, self.site.storage.open(inner_path), ignore_same=False)
                if is_valid:
                    is_modified = False
                else:
                    is_modified = True
            ...
            try:
                self.verifyFile(inner_path, self.site.storage.open(
                    inner_path), ignore_same=False)
                is_modified = False
            except VerifyError:
                is_modified = True
        return is_modified
    ...
    def getFileInfo(self, inner_path, new_file=False):
        ...
        while True:
            ...
            if content and "user_contents" in content:
                ...
                relative_content_path = inner_path[len(
                ...
                if user_auth_address_match:
                    ...
                    back["content_inner_path"] = content_inner_path_dir + \
                        "content.json"
                back["optional"] = None
                back["relative_path"] = "/".join(inner_path_parts)
                return back
    ...
    def getRules(self, inner_path, content=None):
        # Find the files content.json first
    ...
    def getUserContentRules(self, parent_content, inner_path, content):
        ...
        if "inner_path" in parent_content:
            ...
            user_address = re.match(
                r"([A-Za-z0-9]*?)/", inner_path[len(parent_content_dir):]).group(1)
        else:
            user_address = re.match(
                r".*/([A-Za-z0-9]*?)/.*?$", inner_path).group(1)

        try:
            if not content:
                # Read the file if no content specified
                content = self.site.storage.loadJson(inner_path)
            # web/nofish@mojoid.bit
            user_urn = "%s/%s" % (content["cert_auth_type"],
        ...
        if user_address in user_contents["permissions"]:
            rules = copy.copy(user_contents["permissions"].get(
                user_address, {}))  # Default rules based on address
        else:
            rules = copy.copy(user_contents["permissions"].get(
                cert_user_id, {}))  # Default rules based on username
    ...
    def hashFile(self, dir_inner_path, file_relative_path, optional=False):
        ...
        # Calculate sha512 sum of file
        sha512sum = CryptHash.sha512sum(file_path)
        if optional and not self.hashfield.hasHash(sha512sum):
            self.optionalDownloaded(file_inner_path, self.hashfield.getHashId(
                sha512sum), file_size, own=True)

        back[file_relative_path] = {
            "sha512": sha512sum, "size": os.path.getsize(file_path)}
        return back
    ...
    def hashFiles(self, dir_inner_path, ignore_pattern=None, optional_pattern=None):
        ...
        if dir_inner_path and not self.isValidRelativePath(dir_inner_path):
            ignored = True
            self.log.error(
                "- [ERROR] Only ascii encoded directories allowed: %s" % dir_inner_path)
                ...
                if optional:
                    ...
                    files_optional_node.update(
                        self.hashFile(dir_inner_path,
                                      file_relative_path, optional=True)
                    )
                else:
    ...
    def sign(self, inner_path="content.json", privatekey=None, filewrite=True, update_changed_files=False, extend=None, remove_missing_optional=False):
        ...
        if not content:  # Content not exist yet, load default one
            ...
            if inner_path == "content.json":  # It's the root content.json, add some more fields
                content["title"] = "%s - MojoNet_" % self.site.address
                content["description"] = ""
        ...
        files_node, files_optional_node = self.hashFiles(
            helper.getDirname(inner_path), content.get(
                "ignore"), content.get("optional")
        )
        ...
        for file_relative_path, file_details in files_merged.items():
            old_hash = content.get("files", {}).get(
                file_relative_path, {}).get("sha512")
        ...
        if inner_path == "content.json":
            new_content["MojoNet_version"] = config.version
    ...
    def verifyContent(self, inner_path, content):
        content_size = len(json.dumps(content, indent=1)) + sum([file["size"] for file in list(
            content["files"].values()) if file["size"] >= 0])  # Size of new content
        ...
        if old_content:
            old_content_size = len(json.dumps(old_content, indent=1)) + sum(
                [file["size"] for file in list(old_content.get("files", {}).values())])
            old_content_size_optional = sum([file["size"] for file in list(
        ...
        content_size_optional = sum([file["size"] for file in list(
            content.get("files_optional", {}).values()) if file["size"] >= 0])
        # Site size without old content plus the new
        site_size = self.site.settings["size"] - \
            old_content_size + content_size
        # Site size without old content plus the new
        site_size_optional = self.site.settings["size_optional"] - \
            old_content_size_optional + content_size_optional

        site_size_limit = self.site.getSizeLimit() * 1024 * 1024

        # Check site address
        if content.get("address") and content["address"] != self.site.address:
            raise VerifyError("Wrong site address: %s != %s" %
        ...
        if inner_path == "content.json":
            ...
            if content_size_file > site_size_limit:
                ...
                raise VerifyError("Content too large %s B > %s B, aborting task..." % (
        ...
        for file_relative_path in list(content.get("files", {}).keys()) + list(content.get("files_optional", {}).keys()):
            if not self.isValidRelativePath(file_relative_path):
                raise VerifyError("Invalid relative path: %s" %
                                  file_relative_path)
    ...
    def verifyContentInclude(self, inner_path, content, content_size, content_size_optional):
        ...
        if rules.get("max_size") is not None:  # Include size limit
            if content_size > rules["max_size"]:
                raise VerifyError("Include too large %sB > %sB" %
    ...
    def verifyFile(self, inner_path, file, ignore_same=True):
        if inner_path.endswith("content.json"):  # content.json: Check using sign
            ...
            try:
                ...
                if inner_path in self.contents:
                    old_content = self.contents.get(
                        inner_path, {"modified": 0})
                    # Checks if its newer the ours
                    # Ignore, have the same content.json
                ...
                # Content modified in the far future (allow 1 day+)
                ...
                if "sign" in new_content:
                    # The file signed without the sign
                    del(new_content["sign"])
                if "signs" in new_content:
                    # The file signed without the signs
                ...
                if signs:  # New style signing
                    valid_signers = self.getValidSigners(
                        inner_path, new_content)
                    signs_required = self.getSignsRequired(
                        inner_path, new_content)

                    # Check signers_sign on root content.json
                    if inner_path == "content.json" and len(valid_signers) > 1:
                        signers_data = "%s:%s" % (
                            signs_required, ",".join(valid_signers))
                    ...
                    if valid_signs < signs_required:
                        raise VerifyError("Valid signs: %s/%s" %
                                          (valid_signs, signs_required))
                    else:
                ...
                self.log.warning("%s: verify sign error: %s" %
                                 (inner_path, Debug.formatException(err)))
                raise err
