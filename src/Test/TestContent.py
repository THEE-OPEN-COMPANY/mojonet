...
class TestContent:
    ...
    def testInclude(self, site):
        ...
        assert rules["signers"] == [
            "15ik6LeBWnACWfaika1xqGapRZ1zh3JpCo"]  # Valid signer
        ...
        # Extra valid signer defined in parent content.json
        assert "15ik6LeBWnACWfaika1xqGapRZ1zh3JpCo" in valid_signers
        assert "" in valid_signers  # The site itself
        ...
        assert site.content_manager.getValidSigners(
            "content.json") == [""]
    ...
    def testInlcudeLimits(self, site, crypt_bitcoin_lib):
        ...
        # Normal data
        data_dict["signs"] = {"": CryptBitcoin.sign(
            json.dumps(data_dict, sort_keys=True), self.privatekey)}
    ...
    def testSign(self, site, inner_path):
        ...
        with pytest.raises(SignError) as err:
            site.content_manager.sign(
                inner_path, privatekey="5aaa3PvNm5HUWoCfSUfcYvfQ2g3PrRNJWr6Q9eqdBGu23mtMnaa", filewrite=False)
        assert "Private key invalid" in str(err.value)

        # Good privatekey
        content = site.content_manager.sign(
            inner_path, privatekey=self.privatekey, filewrite=False)
        ...
        # Used the site's private key to sign
        ...
        assert (
            {key: val for key, val in content_old.items() if key not in [
                "modified", "signs", "sign", "MojoNet_version"]}
            ==
            {key: val for key, val in content.items() if key not in [
                "modified", "signs", "sign", "MojoNet_version"]}
        )
    ...
    def testSignOptionalFiles(self, site):
        ...
        site.content_manager.contents["content.json"]["optional"] = "((data/img/mojo.*))"
        content_optional = site.content_manager.sign(
            privatekey=self.privatekey, filewrite=False, remove_missing_optional=True)

        del site.content_manager.contents["content.json"]["optional"]
        content_nooptional = site.content_manager.sign(
            privatekey=self.privatekey, filewrite=False, remove_missing_optional=True)

        assert len(content_nooptional.get("files_optional", {})
                   ) == 0  # No optional files if no pattern
        assert len(content_optional["files_optional"]) > 0
        # Hashed optional files should be added to hashfield
        assert len(site.content_manager.hashfield) == len(
            content_optional["files_optional"])
        assert len(content_nooptional["files"]) > len(
            content_optional["files"])

    def testFileInfo(self, site):
        assert "sha512" in site.content_manager.getFileInfo("index.html")
        assert site.content_manager.getFileInfo(
            "data/img/domain.png")["content_inner_path"] == "content.json"
        assert site.content_manager.getFileInfo(
            "data/users/hello.png")["content_inner_path"] == "data/users/content.json"
        assert site.content_manager.getFileInfo(
            "data/users/content.json")["content_inner_path"] == "data/users/content.json"
        assert not site.content_manager.getFileInfo("notexist")

        # Optional file
        file_info_optional = site.content_manager.getFileInfo(
            "data/optional.txt")
        assert "sha512" in file_info_optional
        assert file_info_optional["optional"] is True

        # Not exists yet user content.json
        assert "cert_signers" in site.content_manager.getFileInfo(
            "data/users/unknown/content.json")
    ...
    def testVerify(self, site, crypt_bitcoin_lib):
        ...
        # Re-sign
        data_dict["signs"] = {
            "": CryptBitcoin.sign(json.dumps(data_dict, sort_keys=True), self.privatekey)
        }
        assert site.content_manager.verifyFile(
    ...
    def testVerifyInnerPath(self, site, crypt_bitcoin_lib):
        ...
        for good_relative_path in ["data.json", "out/data.json", "Any File [by none] (1).jpg", "árvzítűrő/tükörfúrógép.txt"]:
            data_dict["files"] = {good_relative_path: {
                "sha512": "369d4e780cc80504285f13774ca327fe725eed2d813aad229e62356b07365906", "size": 505}}
        ...
        for bad_relative_path in ["../data.json", "data/" * 100, "invalid|file.jpg", "con.txt", "any/con.txt"]:
            data_dict["files"] = {bad_relative_path: {
                "sha512": "369d4e780cc80504285f13774ca327fe725eed2d813aad229e62356b07365906", "size": 505}}
    ...
    def testSignUnsafePattern(self, site, key):
        ...
        with pytest.raises(UnsafePatternError) as err:
            site.content_manager.sign(
                "content.json", privatekey=self.privatekey, filewrite=False)
        assert "Potentially unsafe" in str(err.value)

    def testVerifyUnsafePattern(self, site, crypt_bitcoin_lib):
        site.content_manager.contents["content.json"]["includes"][
            "data/test_include/content.json"]["files_allowed"] = "([a-zA-Z]+)*"
        with pytest.raises(UnsafePatternError) as err:
        ...
        site.content_manager.contents["data/users/content.json"]["user_contents"]["permission_rules"]["([a-zA-Z]+)*"] = {
            "max_size": 0}
        with pytest.raises(UnsafePatternError) as err:
            with site.storage.open("data/users//content.json") as data:
                site.content_manager.verifyFile(
                    "data/users//content.json", data, ignore_same=False)
        assert "Potentially unsafe" in str(err.value)
    ...
    def testPathValidation(self, site):
        ...
        assert not site.content_manager.isValidRelativePath(
            "any\\hello.txt")  # \ not allowed
        assert not site.content_manager.isValidRelativePath(
            "/hello.txt")  # Cannot start with /
        assert not site.content_manager.isValidRelativePath(
            "\\hello.txt")  # Cannot start with \
        assert not site.content_manager.isValidRelativePath(
            "../hello.txt")  # Not allowed .. in path
        assert not site.content_manager.isValidRelativePath(
            "\0hello.txt")  # NULL character
        assert not site.content_manager.isValidRelativePath(
            "\31hello.txt")  # 0-31 (ASCII control characters)
        assert not site.content_manager.isValidRelativePath(
            "any/hello.txt ")  # Cannot end with space
        assert not site.content_manager.isValidRelativePath(
            "any/hello.txt.")  # Cannot end with dot
        assert site.content_manager.isValidRelativePath(
            ".hello.txt")  # Allow start with dot
        assert not site.content_manager.isValidRelativePath(
            "any/CON")  # Protected names on Windows
        ...
        assert not site.content_manager.isValidRelativePath(
            "a" * 256)  # Max 255 characters allowed
