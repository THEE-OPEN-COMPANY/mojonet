...
# Don't register shutdown functions to avoid IO error on exit
atexit.register = lambda func: ""
...
# Plugins need to access the configuration
...
config.loadPlugins()
# Parse again to add plugin configuration options
...
config.trackers = []
config.data_dir = TEST_DATA_PATH  # Use test data for unittests
if "MojoNet_LOG_DIR" in os.environ:
    config.log_dir = os.environ["MojoNet_LOG_DIR"]
...
def site_url():
    try:
        ...
        raise pytest.skip("Test requires MojoNet client running: %s" % err)
    return SITE_URL
...
@pytest.fixture()
def db(request):
    db_path = "%s/MojoNet.db" % config.data_dir
    schema = {
        "db_name": "TestDb",
        "db_file": "%s/MojoNet.db" % config.data_dir,
        "maps": {
            "data.json": {
                "to_table": [
                    "test",
