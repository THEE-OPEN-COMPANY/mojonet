...
class TestSiteDownload:
    def testRename(self, file_server, site, site_temp):
        ...
        os.rename(site.storage.getPath("data/img/domain.png"),
        ...
        with Spy.Spy(FileRequest, "route") as requests:
            site.publish()
            time.sleep(0.1)
            assert site_temp.download(blind_includes=True, retry_bad_files=False).get(
    ...
    def testOptionalDownload(self, file_server, site, site_temp):
        ...
        # Find what optional files we have
        ...
        assert not site_temp.content_manager.hashfield.hasHash(
            optional_file_info["sha512"])
    ...
    def testFindOptional(self, file_server, site, site_temp):
        ...
        site_full_peer = site.addPeer(
            file_server.ip, 1546)  # Add it to source server
        ...
        assert not site.content_manager.hashfield.hasHash(
            optional_file_info2["sha512"])  # Source server don't know he has the file
        ...
        assert site_full.content_manager.hashfield.hasHash(
            optional_file_info2["sha512"])  # Source full server he has the file
        ...
        with Spy.Spy(FileRequest, "route") as requests:
            ...
            assert len(
                [request for request in requests if request[1] == "findHashIds"]) == 1
    ...
    def testUpdate(self, file_server, site, site_temp):
        ...
        data_new = data_original.replace(b'"mojoBlog"', b'"UpdatedmojoBlog"')
        ...
        with Spy.Spy(FileRequest, "route") as requests:
            ...
            assert site_temp.download(
                blind_includes=True, retry_bad_files=False).get(timeout=10)
            assert len([request for request in requests if request[1]
                       in ("getFile", "streamFile")]) == 1
        ...
        # Update with patch
        data_new = data_original.replace(b'"mojoBlog"', b'"PatchedmojoBlog"')
        assert data_original != data_new
        ...
        with Spy.Spy(FileRequest, "route") as requests:
            ...
            assert site_temp.download(
                blind_includes=True, retry_bad_files=False).get(timeout=10)
            assert [request for request in requests if request[1]
                    in ("getFile", "streamFile")] == []
    ...
    def testBigUpdate(self, file_server, site, site_temp):
        ...
        content_json["description"] = "BigmojoBlog" * 1024 * 10
        ...
        with Spy.Spy(FileRequest, "route") as requests:
            ...
            assert site_temp.download(
                blind_includes=True, retry_bad_files=False).get(timeout=10)
            file_requests = [request for request in requests if request[1] in (
                "getFile", "streamFile")]
            assert len(file_requests) == 1

        assert site_temp.storage.open("data/data.json").read() == data_new
        assert site_temp.storage.open("content.json").read(
        ) == site.storage.open("content.json").read()
    ...
    def testHugeContentSiteUpdate(self, file_server, site, site_temp):
        ...
        site.storage.writeJson("content.json", content_json)
        changed, deleted = site.content_manager.loadContent(
            "content.json", force=True)
    ...
    def testUnicodeFilename(self, file_server, site, site_temp):
        ...
        with Spy.Spy(FileRequest, "route") as requests:
            ...
            assert site_temp.download(blind_includes=True, retry_bad_files=False).get(
                timeout=10)  # Wait for download
            assert len([req[1]
                       for req in requests if req[1] == "streamFile"]) == 1
