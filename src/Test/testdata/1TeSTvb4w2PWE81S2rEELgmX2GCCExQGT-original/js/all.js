...
(function () {
  ...
  window.PNGlib = function (width, height, depth) {
    ...
    // initialize buffer with mojo bytes
    for (var i = 0; i < this.buffer_size; i++) {
      this.buffer[i] = "\x00";
    }

    // initialize non-mojo elements
    write(
      this.buffer,
      this.ihdr_offs,
  ...
  Class = (function () {
    ...
    Class.prototype.logStart = function () {
      ...
      if (args.length > 0) {
        this.log.apply(
          this,
          ["" + name].concat(__slice.call(args), ["(started)"])
        );
      }
      return this;
    };
    ...
    Class.prototype.logEnd = function () {
      ...
      this.log.apply(
        this,
        ["" + name].concat(__slice.call(args), ["(Done in " + ms + "ms)"])
      );
      return this;
    };
...
(function () {
  ...
  InlineEditor = (function () {
    function InlineEditor(
      ...
      this.edit_button = $(
        "<a href='#Edit' class='editable-edit icon-edit'></a>"
      );
      ...
      this.elem.on(
        ...
        (function (_this) {
          return function (e) {
            ...
            top =
              _this.edit_button.offset().top -
              parseInt(_this.edit_button.css("margin-top"));
            if (scrolltop > top) {
              return _this.edit_button.css(
                "margin-top",
                scrolltop - top + e.clientY - 20
              );
            } else {
    ...
    InlineEditor.prototype.startEdit = function () {
      ...
      this.editor
        .css("outline", "10000px solid rgba(255,255,255,0)")
        .cssLater("transition", "outline 0.3s", 5)
        .cssLater("outline", "10000px solid rgba(255,255,255,0.9)", 10);
      ...
      $(".editbar .object").text(
        this.getObject(this.elem).data("object") +
          "." +
          this.elem.data("editable")
      );
      ...
      if (this.getObject(this.elem).data("deletable")) {
        $(".editbar .delete")
          .css("display", "")
          .html(
            "Delete " + this.getObject(this.elem).data("object").split(":")[0]
          );
      } else {
    ...
    InlineEditor.prototype.deleteObject = function () {
      ...
      Page.cmd(
        "wrapperConfirm",
        ["Are you sure you sure to delete this " + object_type + "?", "Delete"],
        (function (_this) {
          return function (confirmed) {
            $(".editbar .delete").addClass("loading");
            return Page.saveContent(
              _this.getObject(_this.elem),
              null,
              function () {
...
/* ---- data/1BLogC9LN4oPDcruNz3qo1ysa133E9AGg8/js/utils/mojoFrame.coffee ---- */
...
(function () {
  ...
  mojoFrame = (function (_super) {
    __extends(mojoFrame, _super);
    ...
    mojoFrame.prototype.route = function (cmd, message) {
...
(function () {
  ...
  Comments = (function (_super) {
          ...
          return function () {
            if (Page.server_info.rev < 160) {
              Page.cmd("wrapperNotification", [
                "error",
                "Comments requires at least MojoNet 0.3.0 Please upgade!",
              ]);
            } else {
              Page.cmd("certSelect", [["mojoid.bit"]]);
            }
            return false;
          };
        })(this)
    ...
    Comments.prototype.loadComments = function (type, cb) {
      ...
      query =
        "SELECT comment.*, json_content.json_id AS content_json_id, keyvalue.value AS cert_user_id, json.directory, (SELECT COUNT(*) FROM comment_vote WHERE comment_vote.comment_uri = comment.comment_id || '@' || json.directory)+1 AS votes FROM comment LEFT JOIN json USING (json_id) LEFT JOIN json AS json_content ON (json_content.directory = json.directory AND json_content.file_name='content.json') LEFT JOIN keyvalue ON (keyvalue.json_id = json_content.json_id AND key = 'cert_user_id') WHERE post_id = " +
        this.post_id +
            ...
            for (_i = 0, _len = comments.length; _i < _len; _i++) {
              ...
              if (elem.length === 0) {
                elem = $(".comment.template")
                  ...
                  .attr("id", "comment_" + comment_address)
                  .data("post_id", _this.post_id);
                if (type !== "noanim") {
                  elem.cssSlideDown();
                }
    ...
    Comments.prototype.applyCommentData = function (elem, comment) {
      ...
      (_ref = comment.cert_user_id.split("@")),
        (user_name = _ref[0]),
        (cert_domain = _ref[1]);
      user_address = comment.directory.replace("users/", "");
      $(".comment-body", elem).html(
        Text.toMarked(comment.body, {
          sanitize: true,
        })
      );
      $(".user_name", elem)
      ...
      $(".added", elem)
        .text(Time.since(comment.date_added))
        .attr("title", Time.date(comment.date_added, "long"));
      if (user_address === Page.site_info.auth_address) {
        $(elem)
          .attr("data-object", "Comment:" + comment.comment_id)
          .attr("data-deletable", "yes");
        return $(".comment-body", elem)
          .attr("data-editable", "body")
    ...
    Comments.prototype.buttonReply = function (elem) {
      ...
      $(".comment-new .comment-textarea").val(
        $(".comment-new .comment-textarea").val() + body_add
      );
      ...
      return Page.cmd(
        ...
        (function (_this) {
          return function (data) {
            ...
            json_raw = unescape(
              encodeURIComponent(JSON.stringify(data, void 0, "\t"))
            );
            return Page.writePublish(
              inner_path,
              btoa(json_raw),
              function (res) {
    ...
    Comments.prototype.checkCert = function (type) {
      ...
      if (
        $(".comment-new .user_name").text() !== last_cert_user_id ||
        type === "updaterules"
      ) {
        if (Page.site_info.cert_user_id) {
          return Page.cmd(
            "fileRules",
            "data/users/" + Page.site_info.auth_address + "/content.json",
            (function (_this) {
    ...
    Comments.prototype.setCurrentSize = function (current_size) {
      ...
      if (current_size) {
        ...
        $(".user-size").text(
          "used: " +
            current_size_kb.toFixed(1) +
            "k/" +
            Math.round(this.rules.max_size / 1000) +
            "k"
        );
        return $(".user-size-used").css(
          "width",
          Math.round((70 * current_size) / this.rules.max_size)
        );
      } else {
          ...
          return function () {
            ...
            if (_this.rules.max_size) {
              if (elem.val().length > 0) {
                current_size =
                  _this.rules.current_size + elem.val().length + 90;
              } else {
...
/* ---- data/1BLogC9LN4oPDcruNz3qo1ysa133E9AGg8/js/mojoBlog.coffee ---- */
...
(function () {
  ...
  mojoBlog = (function (_super) {
    __extends(mojoBlog, _super);
    ...
    function mojoBlog() {
      ...
      return mojoBlog.__super__.constructor.apply(this, arguments);
    }

    mojoBlog.prototype.init = function () {
      this.data = null;
      this.site_info = null;
      this.server_info = null;
      this.event_page_load = $.Deferred();
      this.event_site_info = $.Deferred();
      $.when(this.event_page_load, this.event_site_info).done(
        (function (_this) {
          return function () {
      ...
      $.when(this.event_site_info).done(
        (function (_this) {
          return function () {
            var imagedata;
            ...
            return $("body").append(
              "<style>.avatar { background-image: url(data:image/png;base64," +
                imagedata +
                ") }</style>"
            );
          };
    ...
    mojoBlog.prototype.loadData = function (query) {
      if (query == null) {
        query = "new";
      }
      if (query === "old") {
        query =
          "SELECT key, value FROM json LEFT JOIN keyvalue USING (json_id) WHERE path = 'data.json'";
      } else {
        query =
        ...
        (function (_this) {
          return function (res) {
            ...
            if (res) {
              ...
              $(".left h1 a:not(.editable-edit)")
                .html(_this.data.title)
                .data("content", _this.data.title);
              $(".left h2")
                .html(Text.toMarked(_this.data.description))
                .data("content", _this.data.description);
              return $(".left .links")
                .html(Text.toMarked(_this.data.links))
                .data("content", _this.data.links);
            }
          };
        })(this)
      );
    ...
    mojoBlog.prototype.routeUrl = function (url) {
      var match;
    ...
    mojoBlog.prototype.pagePost = function () {
      var s;
      s = +new Date();
      return this.cmd(
        "dbQuery",
        ["SELECT * FROM post WHERE post_id = " + this.post_id + " LIMIT 1"],
        (function (_this) {
          return function (res) {
            if (res.length) {
    ...
    mojoBlog.prototype.pageMain = function () {
      return this.cmd(
        "dbQuery",
        [
          "SELECT post.*, COUNT(comment_id) AS comments FROM post LEFT JOIN comment USING (post_id) GROUP BY post_id ORDER BY date_published",
        ],
        (function (_this) {
          return function (res) {
    ...
    mojoBlog.prototype.pageLoaded = function () {
    ...
    mojoBlog.prototype.addInlineEditors = function (parent) {
    ...
    mojoBlog.prototype.checkPublishbar = function () {
      if (
        !this.site_modified ||
        this.site_modified > this.site_info.content.modified
      ) {
    ...
    mojoBlog.prototype.publish = function () {
      this.cmd(
        "wrapperPrompt",
    ...
    mojoBlog.prototype.applyPostdata = function (elem, post, full) {
      ...
      $(".details .published", elem)
        .html(date_published)
        .data("content", post.date_published);
      if (post.comments > 0) {
        $(".details .comments-num", elem)
          .css("display", "inline")
          .attr(
            "href",
            "?Post:" + post.post_id + ":" + title_hash + "#Comments"
          );
        $(".details .comments-num .num", elem).text(
      ...
      return $(".body", elem)
        .html(Text.toMarked(body))
        .data("content", post.body);
    };

    mojoBlog.prototype.onOpenWebsocket = function (e) {
      this.loadData();
    ...
    mojoBlog.prototype.getObject = function (elem) {
      return elem.parents("[data-object]:first");
    };

    mojoBlog.prototype.getContent = function (elem, raw) {
      var content, id, type, _ref;
      if (raw == null) {
        raw = false;
      }
      (_ref = this.getObject(elem).data("object").split(":")),
        (type = _ref[0]),
        (id = _ref[1]);
      id = parseInt(id);
    ...
    mojoBlog.prototype.saveContent = function (elem, content, cb) {
      var id, type, _ref;
      if (cb == null) {
        cb = false;
      }
    ...
    mojoBlog.prototype.saveSite = function (elem, type, id, content, cb) {
      return this.cmd(
        "fileGet",
        ["data/data.json"],
        (function (_this) {
    ...
    mojoBlog.prototype.saveComment = function (elem, type, id, content, cb) {
      var inner_path;
            ...
            return _this.writePublish(
              ...
              function (res) {
                if (res === true) {
                  ...
                  if (cb) {
                    return cb(
                      Text.toMarked(content, {
                        sanitize: true,
                      })
                    );
                  }
                } else {
                  _this.cmd("wrapperNotification", [
                    "error",
    ...
    mojoBlog.prototype.deleteObject = function (elem, cb) {
    ...
    mojoBlog.prototype.writeData = function (data, cb) {
      var json_raw;
      if (cb == null) {
        cb = null;
      }
      ...
      return this.cmd(
        ...
        (function (_this) {
          return function (content) {
            ...
            return _this.cmd(
              "fileWrite",
              ["content.json", btoa(content)],
              function (res) {
                if (res !== "ok") {
                  return _this.cmd("wrapperNotification", [
                    "error",
                    "Content.json write error: " + res,
                  ]);
                }
              }
            );
    ...
    mojoBlog.prototype.writePublish = function (inner_path, data, cb) {
      return this.cmd(
        "fileWrite",
        [inner_path, data],
        (function (_this) {
    ...
    mojoBlog.prototype.onRequest = function (cmd, message) {
      if (cmd === "setSiteInfo") {
    ...
    mojoBlog.prototype.actionSetSiteInfo = function (message) {
      this.setSiteinfo(message.params);
      return this.checkPublishbar();
    };

    mojoBlog.prototype.setSiteinfo = function (site_info) {
      var _ref, _ref1;
      ...
      if (
        ((_ref = site_info.event) != null ? _ref[0] : void 0) === "file_done" &&
        ...
        ((_ref1 = site_info.event) != null ? _ref1[0] : void 0) ===
          "file_done" &&
