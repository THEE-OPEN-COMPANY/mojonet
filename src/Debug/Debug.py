...
python_lib_dirs = [path.replace("\\", "/") for path in sys.path if re.sub(
    r".*[\\/]", "", path) in ("site-packages", "dist-packages")]
# TODO: check if returns the correct path for PyPy
        ...
        if path.startswith("src/gevent/"):
            ...
            for base in python_lib_dirs:
                ...
                for base in (root_dir + "/src", root_dir + "/plugins", root_dir):
                    ...
                    file_title = path
                    for needle in ("/MojoNet/", "/core/"):
                        if needle in file_title.lower():
                            file_title = "?/" + \
                                file_title[file_title.lower().rindex(
                                    needle) + len(needle):]
...
def formatException(err=None, format="text"):
    ...
    tb = formatTraceback([[frame[0], frame[1]]
...
def formatStack(limit=None):
    import inspect
    tb = formatTraceback([[frame[1], frame[2]]
                         for frame in inspect.stack()[1:]], limit=limit)
    return " > ".join(tb)
...
def testBlock():
    ...
    while 1:
        ...
        if time.time() - last_time > 1.1:
            logging.debug("Gevent block detected: %.3fs" %
                          (time.time() - last_time - 1))
            num_block += 1
        last_time = time.time()
