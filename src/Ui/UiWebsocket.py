...
class UiWebsocket(object):
            ...
            if message:
                try:
                    ...
                    if not self.hasPlugin("Multiuser"):
                        self.cmd("error", "Internal error: %s" %
    ...
    def send(self, message, cb=None):
        # Add message id to allow response
    ...
    def formatSiteInfo(self, site, create_user=True):
        ...
        if site.settings["own"]:
            ret["privatekey"] = bool(self.user.getSiteData(
                site.address, create=create_user).get("privatekey"))
    ...
    def actionAs(self, to, address, cmd, params=[]):
        ...
        # Use the same permissions as current site
    ...
    def actionResponse(self, to, result):
        if to in self.waiting_cb:
            ...
            self.log.error("Websocket callback not found: %s, %s" %
                           (to, result))
        ...
        for site in list(self.server.sites.values()):
            for tracker, stats in site.announcer.stats.items():
                ...
                if tracker not in back:
                    back[tracker] = {}
                is_latest_data = bool(stats["time_request"] > back[tracker].get(
                    "time_request", 0) and stats["status"])
    ...
    def actionSiteSign(self, to, privatekey=None, inner_path="content.json", remove_missing_optional=False, update_changed_files=False, response_ok=True):
        ...
        if is_user_content and privatekey is None:
            ...
            self.log.debug("Extending content.json with cert %s" %
                           extend["cert_user_id"])
        ...
        if privatekey == "stored":  # Get privatekey from sites.json
            ...
            if not privatekey:
                self.cmd("notification", ["error", _[
                         "Content signing failed"] + "<br><small>Private key not found in sites.json </small>"])
                self.response(
                    to, {"error": "Site sign failed: Private key not stored."})
                self.log.error(
                    "Site sign failed: %s: Private key not stored in sites.json" % inner_path)
                return
        ...
        try:
            site.content_manager.sign(inner_path, privatekey, extend=extend,
                                      update_changed_files=update_changed_files, remove_missing_optional=remove_missing_optional)
        except (VerifyError, SignError) as err:
            self.cmd("notification", ["error", _[
                     "Content signing failed"] + "<br><small>%s</small>" % err])
            ...
            self.cmd("notification", ["error", _[
                     "Content signing error"] + "<br><small>%s</small>" % Debug.formatException(err)])
            self.response(to, {"error": "Site sign error: %s" %
                          Debug.formatException(err)})
            self.log.error("Site sign error: %s: %s" %
                           (inner_path, Debug.formatException(err)))
            return

        # Load new content.json, ignore errors
    ...
    def actionSitePublish(self, to, privatekey=None, inner_path="content.json", sign=True, remove_missing_optional=False, update_changed_files=False):
        ...
        if called_instantly:  # Allowed to call instantly
            ...
            self.cmd("progress", ["publish", _[
                     "Content published to {0}/{1} peers."].format(0, 5), 0])
            thread.link(lambda thread: self.cbSitePublish(
                to, self.site, thread, notification, callback=notification))
        else:
            self.cmd(
                "notification",
                ["info", _["Content publish queued for {0:.0f} seconds."].format(
    ...
    def doSitePublish(self, site, inner_path):
        ...
        back = site.publish(limit=5, inner_path=inner_path,
                            diffs=diffs, cb_progress=cbProgress)
        if back == 0:  # Failed to publish to anyone
            self.cmd("progress", ["publish", _[
                     "Content publish failed."], -100])
        else:
                ...
                if any(main.file_server.port_opened.values()) or main.file_server.tor_manager.start_onions:
                    if notification:
                        self.cmd("notification", [
                                 "info", _["No peers found, but your content is ready to access."]])
                    if callback:
                        self.response(to, "ok")
                else:
                    if notification:
    ...
    def actionFileWrite(self, to, inner_path, content_base64, ignore_bad_files=False):
        ...
        # Also check the content.json from same directory
        ...
        if (self.site.bad_files.get(inner_path) or self.site.bad_files.get(content_inner_path)) and not ignore_bad_files:
            ...
            if not found:
                self.cmd(
                    "confirm",
                    [_["This file still in sync, if you write it now, then the previous content may be lost."], _[
                        "Write content anyway"]],
        ...
        try:
            ...
            if (
                ...
                self.site.storage.isFile(
                    inner_path) and not self.site.storage.isFile(inner_path + "-old")
            ):
                try:
        ...
        for ws in self.site.websockets:
            if ws != self:
                ws.event("siteChanged", self.site, {
    ...
    def actionFileDelete(self, to, inner_path):
        ...
        if file_info and file_info.get("optional"):
            ...
            if relative_path in content_json.get("files_optional", {}):
                ...
                # File sill exists after removing from content.json (owned site)
    ...
    def actionFileList(self, to, inner_path):
        try:
            ...
            self.log.error("fileList %s error: %s" %
                           (inner_path, Debug.formatException(err)))
        ...
        try:
            if stats:
                ...
                for file_name in self.site.storage.list(inner_path):
                    file_stats = os.stat(self.site.storage.getPath(
            ...
            self.log.error("dirList %s error: %s" %
                           (inner_path, Debug.formatException(err)))
            return {"error": Debug.formatExceptionMessage(err)}
    ...
    def actionFileGet(self, to, inner_path, required=True, format="text", timeout=300, priority=6):
        try:
            ...
            self.log.debug("%s fileGet error: %s" %
                           (inner_path, Debug.formatException(err)))
            body = None
    ...
    def actionFileRules(self, to, inner_path, use_my_cert=False, content=None):
        ...
        if inner_path.endswith("content.json") and rules:
            if content:
                rules["current_size"] = len(json.dumps(
                    content)) + sum([file["size"] for file in list(content.get("files", {}).values())])
            else:
    ...
    def actionCertAdd(self, to, domain, auth_type, auth_user_name, cert):
        try:
            res = self.user.addCert(self.user.getAuthAddress(
                self.site.address), domain, auth_type, auth_user_name, cert)
            if res is True:
                self.cmd(
                    "notification",
                    ["done", _(
                        "{_[New certificate added]:} <b>{auth_type}/{auth_user_name}@{domain}</b>.")]
                )
                ...
                self.cmd(
                    "confirm",
                    [body, _(
                        "Change it to {auth_type}/{auth_user_name}@{domain}")],
            ...
            self.log.error("CertAdd error: Exception - %s (%s)" %
    ...
    def cbCertAddConfirm(self, to, domain, auth_type, auth_user_name, cert):
        ...
        self.user.addCert(self.user.getAuthAddress(
            self.site.address), domain, auth_type, auth_user_name, cert)
        self.cmd(
            "notification",
            ["done", _(
                "Certificate changed to: <b>{auth_type}/{auth_user_name}@{domain}</b>.")]
        )
        self.user.setCert(self.site.address, domain)
        self.site.updateWebsocket(cert_changed=domain)
        self.response(to, "ok")
    ...
    def actionCertSelect(self, to, accepted_domains=[], accept_any=False, accepted_pattern=None):
        ...
        site_data = self.user.getSiteData(
            self.site.address)  # Current auth address
        ...
        body = "<span style='padding-bottom: 5px; display: inline-block'>" + \
            _["Select account you want to use in this site:"] + "</span>"
        # Accounts
        for domain, account, css_class in accounts:
            if domain == active:
                css_class += " active"  # Currently selected option
                title = _(
                    "<b>%s</b> <small>({_[currently selected]})</small>") % account
            else:
                title = "<b>%s</b>" % account
        ...
        more_domains = [
            domain for domain in accepted_domains if domain not in self.user.certs]
        if more_domains:
        ...
        script = """
             $(".notification .select.cert").on("click", function() {
                ...
                mojoframe.response(%s, this.title)
                return false
             })
    ...
    def actionPermissionDetails(self, to, permission):
        if permission == "ADMIN":
            self.response(to, _["Modify your client's configuration and access all site"] +
                          " <span style='color: red'>" + _["(Dangerous!)"] + "</span>")
    ...
    def actionSiteList(self, to, connecting_sites=False):
        ...
        for site in list(self.server.sites.values()):
            ...
            # Dont generate the auth_address on listing
    ...
    def actionSiteUpdate(self, to, address, check_files=False, since=None, announce=False):
        def updateThread():
            site.update(announce=announce,
                        check_files=check_files, since=since)
    ...
    def cbSiteClone(self, to, address, root_inner_path="", target_address=None, redirect=True):
        ...
        if target_address:
            ...
            privatekey = self.user.getSiteData(
                target_site.address).get("privatekey")
            site.clone(target_address, privatekey,
            ...
            new_site = site.clone(
                new_address, new_site_data["privatekey"], address_index=new_address_index, root_inner_path=root_inner_path)
        ...
        if site.bad_files:
            for bad_inner_path in list(site.bad_files.keys()):
                ...
                if not is_user_file and bad_inner_path != "content.json":
                    self.cmd("notification", ["error", _[
                             "Clone error: Site still in sync"]])
                    return {"error": "Site still in sync"}

        if "ADMIN" in self.getPermissions(to):
            self.cbSiteClone(to, address, root_inner_path,
                             target_address, redirect)
        else:
            self.cmd(
                "confirm",
    ...
    def actionSiteListModifiedFiles(self, to, content_inner_path="content.json"):
        ...
        inner_paths = [content_inner_path] + list(content.get(
            "includes", {}).keys()) + list(content.get("files", {}).keys())

        if len(inner_paths) > 100:
            return {"error": "Too many files in content.json"}

        for relative_inner_path in inner_paths:
            inner_path = helper.getDirname(
                content_inner_path) + relative_inner_path
            try:
                is_mtime_newer = os.path.getmtime(
                    self.site.storage.getPath(inner_path)) > min_mtime + 1
                if is_mtime_newer:
                    if inner_path.endswith("content.json"):
                        ...
                        is_same_size = self.site.storage.getSize(
                            inner_path) == previous_size
                        ext = inner_path.rsplit(".", 1)[-1]
                        is_text_file = ext in [
                            "json", "txt", "html", "js", "css"]
                        if is_same_size:
                            if is_text_file:
                                is_modified = self.site.content_manager.isModified(
                                    inner_path)  # Check sha512 hash
                            else:
                                is_modified = False
                        else:
                            is_modified = True
    ...
    def actionUserGetSettings(self, to):
        settings = self.user.sites.get(
            self.site.address, {}).get("settings", {})
    ...
    def actionServerUpdate(self, to):
        def cbServerUpdate(res):
            ...
            for websocket in self.server.websockets:
                websocket.cmd(
                    "notification",
                    ["info", _[
                        "Updating MojoNet client, will be back in a few minutes..."], 20000]
                )
        ...
        self.cmd(
            "confirm",
            [_["Update <b>MojoNet client</b> to latest version?"], _["Update"]],
            cbServerUpdate
        )
    ...
    def actionServerShutdown(self, to, restart=False):
        ...
        if restart:
            message = [_["Restart <b>MojoNet client</b>?"], _["Restart"]]
        else:
            message = [_["Shut down <b>MojoNet client</b>?"], _["Shut down"]]
    ...
    def actionServerShowdirectory(self, to, directory="backup", inner_path=""):
        ...
        if directory == "backup":
            ...
            path = os.path.abspath(self.site.storage.getPath(
    ...
    def actionConfigSet(self, to, key, value):
        ...
        if key == "offline":
            if value:
                ...
                gevent.spawn(main.file_server.checkSites,
                             check_files=False, force_port_check=True)
