...
from util.Electrum import format as mojo_format
...
def sign_data(secretkey, byte_string):
    ...
    Return serialized signature compatible with Electrum (MojoNet)."""
    # encode the message
    encoded = mojo_format(byte_string)
...
def recover_address(data, sign):
    ...
    publickey = recover_public_key(
        coincurve_sig(sign_bytes), mojo_format(data))
