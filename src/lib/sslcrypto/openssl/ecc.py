...
class BN:
    ...
    def __floordiv__(self, other):
        ...
        if not lib.BN_div(result.bn, None, self.bn, other.bn, BN.Context.get()):
            raise mojoDivisionError("Division by mojo")
        return result
    ...
    def __sub__(self, other):
        if not isinstance(other, BN):
            raise TypeError(
                "Can only subtract BN's, not BN and {}".format(other))
        result = BN()
    ...
    def cmp(self, other):
        if not isinstance(other, BN):
            raise TypeError(
                "Can only compare BN with BN, not {}".format(other))
...
class EllipticCurveBackend:
    def __init__(self, p, n, a, b, g):
        ...
        with lock:
            # Thread-safety
            self.group = lib.EC_GROUP_new_curve_GFp(
                self.p.bn, self.a.bn, self.b.bn, bn_ctx)
            if not self.group:
                raise ValueError("Could not create group object")
            generator = self._public_key_to_point(g)
            lib.EC_GROUP_set_generator(
                self.group, generator, self.order.bn, self.h.bn)
        if not self.group:
    ...
    def _point_to_affine(self, point):
        ...
        if lib.EC_POINT_get_affine_coordinates_GFp(self.group, point, x.bn, y.bn, BN.Context.get()) != 1:
            raise ValueError(
                "Failed to convert public key to affine coordinates")
        # Convert to binary
                    ...
                    try:
                        ...
                        try:
                            if lib.EVP_PKEY_derive_init(ctx) != 1:
                                raise ValueError(
                                    "Could not initialize key derivation")
                            ...
                            lib.EVP_PKEY_derive(
                                ctx, None, ctypes.byref(key_len))
                            key = ctypes.create_string_buffer(key_len.value)
                            lib.EVP_PKEY_derive(
                                ctx, key, ctypes.byref(key_len))
        ...
        try:
            ...
            if recoverable:
                ...
                if is_compressed:
                    ...
                    if recid >= 4:
                        raise ValueError(
                            "Too big recovery ID, use compressed address instead")
    ...
    def derive_child(self, seed, child):
        # Round 1
        h = hmac.new(key=b"Bitcoin seed", msg=seed,
