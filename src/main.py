...
# If set True then update and restart MojoNet after main loop ended
update_after_shutdown = False
# If set True then restart MojoNet after main loop ended
...
if not os.path.isdir(config.data_dir):
    ...
    try:
        ...
        startupError("Can't change permission of %s: %s" %
                     (config.data_dir, err))
...
if config.action == "main":
    ...
    try:
        ...
        startupError(
            "Can't open lock file, your MojoNet client is probably already running, exiting... (%s)" % err)
...
if sys.platform.startswith("win"):
    import subprocess
    try:
        chcp_res = subprocess.check_output(
            "chcp 65001", shell=True).decode(errors="ignore").strip()
...
if config.proxy:
    ...
    if config.fileserver_ip == "*":
        # Do not accept connections anywhere but localhost
...
class Actions(object):
    def call(self, function_name, kwargs):
        logging.info("Version: %s r%s, Python %s, Gevent: %s" % (
    ...
    def main(self):
        ...
        gevent.joinall([gevent.spawn(ui_server.start),
    ...
    def siteCreate(self, use_master_seed=True):
        ...
        if use_master_seed:
            ...
            logging.info(
                "Generated using master seed from users.json, site index: %s" % address_index)
        else:
    ...
    def siteVerify(self, address):
        ...
        for content_inner_path in site.content_manager.contents:
            ...
            if file_correct is True:
                logging.info("[OK] %s (Done in %.3fs)" %
                             (content_inner_path, time.time() - s))
            else:
                logging.error("[ERROR] %s: invalid file: %s!" %
                              (content_inner_path, err))
                input("Continue?")
        ...
        if not bad_files:
            logging.info("[OK] All file sha512sum matches! (%.3fs)" %
                         (time.time() - s))
        else:
    ...
    def sitePublish(self, address, peer_ip=None, peer_port=15441, inner_path="content.json"):
        ...
        # Serving the site even if its disabled
        ...
        try:
            ...
            logging.info("Creating FileServer....")
            # Dont check every site integrity
    ...
    def cryptGetPrivatekey(self, master_seed, site_address_index=None):
        ...
        if len(master_seed) != 64:
            logging.error(
                "Error: Invalid master seed length: %s (required: 64)" % len(master_seed))
            return False
    ...
    def test(self, test_name, *args, **kwargs):
        ...
        test_names = [funcToName(name) for name in dir(
            self) if name.startswith("test") and name != "test"]
        if not test_name:
...
actions = Actions()
# Starts here when running MojoNet.py
