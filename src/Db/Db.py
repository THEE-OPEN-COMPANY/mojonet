...
class Db(object):
    ...
    def connect(self):
        ...
        try:
            ...
            self.conn = sqlite3.connect(
                self.db_path, isolation_level="DEFERRED", check_same_thread=False)
            ...
            self.log.debug("Connect by thread: %s" %
    ...
    def commit(self, reason="Unknown"):
        ...
        try:
            ...
            self.commiting = True
            self.conn.commit()
            self.log.debug("Commited in %.3fs (reason: %s)" %
                           (time.time() - s, reason))
            return True
        except Exception as err:
            if "SQL statements in progress" in str(err):
                self.log.warning("Commit delayed: %s (reason: %s)" %
                                 (Debug.formatException(err), reason))
            else:
                self.log.error("Commit error: %s (reason: %s)" %
                               (Debug.formatException(err), reason))
            return False
        finally:
            self.commiting = False
    ...
    def processDelayed(self):
        ...
        if len(self.delayed_queue) > 10:
            self.log.debug("Processed %s delayed queue in %.3fs" %
    ...
    def close(self, reason="Unknown"):
        ...
        self.conn = None
        self.cur = None
        self.log.debug("%s closed (reason: %s) in %.3fs, opened: %s" % (
    ...
    def getTableVersion(self, table_name):
        if not self.db_keyvalues:  # Get db keyvalues
            try:
                # json_id = 0 is internal keyvalues
    ...
    def checkTables(self):
        ...
        for table_name, table_settings in self.schema.get("tables", {}).items():
            try:
                ...
                self.log.error("Error creating table %s: %s" %
                               (table_name, Debug.formatException(err)))
                raise DbTableError(err, table_name)

        self.log.debug("Db check done in %.3fs, changed tables: %s" %
    ...
    def updateJson(self, file_path, file=None, cur=None):
        ...
        # File path realative to db file
        ...
        for dbmap in matched_maps:
            ...
            if dbmap.get("to_keyvalue"):
                # Get current values
                res = cur.execute(
                    "SELECT * FROM keyvalue WHERE json_id = ?", (json_row["json_id"],))
                current_keyvalue = {}
                current_keyvalue_id = {}
                for row in res:
                ...
                for key in dbmap["to_keyvalue"]:
                    if key not in current_keyvalue:  # Keyvalue not exist yet in the db
                        cur.execute(
                            ...
                            {"key": key, "value": data.get(
                                key), "json_id": json_row["json_id"]}
                        )
                    # Keyvalue different value
            ...
            if dbmap.get("to_json_table"):
                ...
                data_json_row = dict(cur.getJsonRow(
                ...
                if changed:
                    ...
                    data_json_row.update(
                        {key: val for key, val in data.items() if key in dbmap["to_json_table"]})
            ...
            for table_settings in dbmap.get("to_table", []):
                if isinstance(table_settings, dict):  # Custom settings
                    # Table name to insert datas
                    table_name = table_settings["table"]
                    # Node keyname in data json file
                    node = table_settings.get("node", table_name)
                    # Map dict key as this col
                    key_col = table_settings.get("key_col")
                    # Map dict value as this col
                ...
                if not import_cols:
                    import_cols = set(
                        [item[0] for item in self.schema["tables"][table_name]["cols"]])

                cur.execute("DELETE FROM %s WHERE json_id = ?" %
                            table_name, (json_row["json_id"],))
                ...
                if key_col:  # Map as dict
                    for key, val in data[node].items():
                        if val_col:  # Single value
                            ...
                            if type(val) is dict:  # Single row
                                row = val
                                if import_cols:
                                    # Filter row by import_cols
                                    row = {key: row[key]
                                           for key in row if key in import_cols}
                                row[key_col] = key
                    ...
                    for row in data[node]:
                        ...
                        if import_cols:
                            ...
                            row = {key: row[key]
                                   for key in row if key in import_cols}
                        cur.execute("INSERT OR REPLACE INTO %s ?" %
                                    table_name, row)

        # Cleanup json row
        if not data:
            self.log.debug("Cleanup json row for %s" % file_path)
            cur.execute("DELETE FROM json WHERE json_id = %s" %
                        json_row["json_id"])
...
if __name__ == "__main__":
    ...
    dbjson = Db(json.load(open("mojotalk.schema.json")),
                "data/users/mojotalk.db")
