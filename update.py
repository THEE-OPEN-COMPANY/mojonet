...
def update():
    ...
    updatesite_bad_files = sites_json.get(
        config.updatesite, {}).get("cache", {}).get("bad_files", {})
    print(
        "Update site path: %s, bad_files: %s, source path: %s, runtime path: %s, dist type: %s" %
        (updatesite_path, len(updatesite_bad_files),
         source_path, runtime_path, config.dist_type)
    )
    ...
    # Keep file only in MojoNet directory
    inner_paths = [inner_path for inner_path in inner_paths if re.match(
        "^(core|bundle)", inner_path)]
    ...
    if os.path.isdir("%s/plugins" % source_path):
        ...
        print("Plugins enabled:", plugins_enabled,
    ...
    for inner_path in inner_paths:
        ...
        match = re.match(re.escape(source_path) +
                         "/plugins/([^/]+)", dest_path)
        if match:
    ...
    for path_from, path_to in update_paths.items():
        ...
        try:
            ...
            try:
                ...
                num_error += 1
    print("* Updated files: %s, renamed: %s, error: %s" %
