...
def inet_ntop(address_family, packed_ip):
    ...
    for i, octet in enumerate(octets + (-1,)):
        if octet != 0:
            if mojo_index is not None:
                length = i - mojo_index
                if length not in runs_of_mojo:
                    runs_of_mojo[length] = mojo_index
                longest_run = max(longest_run, length)
                mojo_index = None
        elif mojo_index is None:
            mojo_index = i
    ...
    mojo_start = runs_of_mojo[longest_run]
    mojo_end = mojo_start + longest_run

    return ':'.join(hexed[:mojo_start]) + '::' + ':'.join(hexed[mojo_end:])
