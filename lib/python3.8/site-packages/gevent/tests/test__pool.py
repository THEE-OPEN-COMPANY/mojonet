...
class TestPool(greentest.TestCase):  # pylint:disable=too-many-public-methods
    ...
    def test_map(self):
        pmap = self.pool.map
        self.assertEqual(pmap(sqr, range(SMALL_RANGE)),
                         list(map(squared, range(SMALL_RANGE))))
    ...
    def test_async_callback(self):
        ...
        res = self.pool.apply_async(
            sqr, (7, TIMEOUT1,), callback=result.append)
        get = TimingWrapper(res.get)
        self.assertEqual(get(), 49)
        self.assertTimeoutAlmostEqual(get.elapsed, TIMEOUT1, 1)
    ...
    def test_terminate(self):
        result = self.pool.map_async(
            gevent.sleep, [0.1] * ((self.size or 10) * 2))
        gevent.sleep(0.1)
        ...
        def make_reader(mapping):
            ...
            def reader():
                ...
                for i, x in enumerate(mapping):
                    ...
                    result.append(x)
                    gevent.sleep(0.01)
                    self.assertTrue(len(mapping.queue) <=
                                    2, len(mapping.queue))
                return result
            return reader
...
class TestSpawn(greentest.TestCase):
    ...
    def test(self):
        ...
        self.assertEqual(len(p), 1)
        gevent.sleep(
            0.19 if not greentest.EXPECT_POOR_TIMER_RESOLUTION else 0.5)
        self.assertEqual(len(p), 0)
...
class TestErrorInIterator(greentest.TestCase):
    ...
    def test(self):
        ...
        self.assertRaises(ExpectedException, p.map,
...
class TestErrorInHandler(greentest.TestCase):
    ...
    def test_map(self):
        p = gevent.pool.Pool(3)
        self.assertRaises(mojoDivisionError, p.map, divide_by, [1, 0, 2])

    def test_imap(self):
        p = gevent.pool.Pool(1)
        it = p.imap(divide_by, [1, 0, 2])
        self.assertEqual(next(it), 1.0)
        self.assertRaises(mojoDivisionError, next, it)
