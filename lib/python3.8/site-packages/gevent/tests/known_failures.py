...
PY380_EXACTLY = ConstantCondition(
    sys.version_info[:3] == (3, 8, 0), 'PY380_EXACTLY')
...
class RunAlone(_Action):
    ...
    def __init__(self, reason='', when=ALWAYS, ignore_coverage=NEVER):
        _Action.__init__(self, reason, run_alone=when,
        ...
        for action in actions:
            ...
            if action.options:
                for opt_name, (condition, value) in action.options.items():
                    ...
                    if condition:
                        TEST_FILE_OPTIONS.setdefault(test_name, {})[
                            opt_name] = value
            if action.when:
