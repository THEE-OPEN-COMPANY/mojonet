...
class _AbstractPoolTest(TestCase):
    ...
    def test_map(self):
        pmap = self.pool.map
        if self.MAP_IS_GEN:
            def pmap(f, i): return list(self.pool.map(f, i))
...
class TestSpawn(TestCase):
    ...
    def test_basics(self):
        ...
        log = []
        def sleep_n_log(item, seconds): return [
            sleep(seconds), log.append(item)]
...
class TestErrorInIterator(TestCase):
    ...
    def test(self):
        ...
        self.assertRaises(greentest.ExpectedException,
                          self.pool.map, lambda x: None, error_iter())
        gevent.sleep(0.001)
...
class TestThreadResult(greentest.TestCase):
    ...
    def test_exception_in_on_async_doesnt_crash(self):
        ...
        tr = gevent.threadpool.ThreadResult(
            None, gevent.get_hub(), bad_when_ready)
