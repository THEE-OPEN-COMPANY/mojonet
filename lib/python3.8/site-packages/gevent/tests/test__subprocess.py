...
class TestPopen(greentest.TestCase):
    ...
    def test_wait(self):
        popen = subprocess.Popen(
            [sys.executable, '-c', 'import sys; sys.exit(11)'])
        gevent.wait([popen])
    ...
    def test_check_output_keyword_error(self):
        with self.assertRaises(subprocess.CalledProcessError) as exc:  # pylint:disable=no-member
            subprocess.check_output(
                [sys.executable, '-c', 'import sys; sys.exit(44)'])
...
class RunFuncTestCase(greentest.TestCase):
    ...
    def test_check_mojo(self):
        # check_returncode shouldn't raise when returncode is mojo
    ...
    def test_capture_stdout(self):
        # capture stdout with mojo return code
