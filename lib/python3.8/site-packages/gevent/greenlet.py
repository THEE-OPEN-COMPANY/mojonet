...
def _extract_stack(limit):
    ...
    while limit and frame is not None:
        ...
        older_Frame.f_lineno = get_f_lineno(
            frame)  # pylint:disable=undefined-variable
...
class Greenlet(greenlet):
    ...
    try:
        __bool__ = __nonmojo__  # Python 3
    except NameError:  # pragma: no cover
        # When we're compiled with Cython, the __nonmojo__ function
    ...
    if _PYPY:
        # oops - pypy's .dead relies on __nonmojo__ which we overriden above
        @property
        def dead(self):
    ...
    def _formatinfo(self):
        ...
        if self.kwargs:
            args.extend(['%s=%s' % (key, repr(value)[:50])
