...
class ThreadPool(GroupMappingMixin):
    ...
    def _set_size(self, size):
        if size < 0:
            raise ValueError(
                'Size of the pool cannot be negative: %r' % (size, ))
        if size > self._maxsize:
            raise ValueError('Size of the pool cannot be bigger than maxsize: %r > %r' % (
                size, self._maxsize))
        if self.manager:
            self.manager.kill()
    ...
    def _ignore_error(future_proxy, fn):
        def cbwrap(_):
            ...
            try:
                ...
                future_proxy.hub.print_exception(
                    (fn, future_proxy), None, None, None)
        return cbwrap
