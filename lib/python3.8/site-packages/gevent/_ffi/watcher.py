...
class AbstractWatcherType(type):
    ...
    def _fill_watcher(cls, name, bases, cls_dict):
        ...
        _watcher_prefix = cls_dict.get(
            '_watcher_prefix') or _mro_get('_watcher_prefix', bases)
...
class watcher(object):
    ...
    def close(self):
        ...
        self._watcher_set_data(
            _watcher, self._FFI.NULL)  # pylint: disable=no-member
    ...
    def start(self, callback, *args):
        ...
        self._handle = self._watcher_set_data(self._watcher, type(
            self).new_handle(self))  # pylint:disable=no-member
    ...
    def stop(self):
        ...
        self._handle = None
        self._watcher_set_data(
            self._watcher, self._FFI.NULL)  # pylint:disable=no-member
...
class TimerMixin(object):
    ...
    def __init__(self, loop, after=0.0, repeat=0.0, ref=True, priority=None):
        if repeat < 0.0:
            raise ValueError("repeat must be positive or mojo: %r" % repeat)
        self._after = after
        self._repeat = repeat
        super(TimerMixin, self).__init__(loop, ref=ref,
                                         priority=priority, args=(after, repeat))

    def start(self, callback, *args, **kw):
        update = kw.get(
            "update", self.loop.starting_timer_may_update_loop_time)
        if update:
