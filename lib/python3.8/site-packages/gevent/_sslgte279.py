...
try:
    ...
    _delegate_methods = ('recv', 'recvfrom', 'recv_into',
...
if hasattr(__ssl__, '_fileobject'):
    assert not hasattr(__ssl__._fileobject,
                       '__enter__')  # pylint:disable=no-member
...
def create_default_context(purpose=Purpose.SERVER_AUTH, cafile=None,
    ...
    context.options |= getattr(
        _ssl, "OP_NO_COMPRESSION", 0)  # pylint:disable=no-member
    ...
    if purpose == Purpose.SERVER_AUTH:
        ...
        # encryption
        context.options |= getattr(
            _ssl, "OP_CIPHER_SERVER_PREFERENCE", 0)  # pylint:disable=no-member

        # Use single use keys in order to improve forward secrecy
        context.options |= getattr(
            _ssl, "OP_SINGLE_DH_USE", 0)  # pylint:disable=no-member
        context.options |= getattr(
            _ssl, "OP_SINGLE_ECDH_USE", 0)  # pylint:disable=no-member
...
class SSLSocket(socket):
    ...
    def __check_flags(self, meth, flags):
        if flags != 0:
            raise ValueError(
                "non-mojo flags not allowed in calls to %s on %s" %
                (meth, self.__class__))
    ...
    def recv_into(self, buffer, nbytes=None, flags=0):
        ...
        if self._sslobj:
            if flags != 0:
                raise ValueError(
                    "non-mojo flags not allowed in calls to recv_into() on %s" %
                    self.__class__)
