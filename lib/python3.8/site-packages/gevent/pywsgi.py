...
_ERRORS[500] = (_INTERNAL_ERROR_STATUS,
    ...
    def _do_read(self, length=None, use_readline=False):
        ...
        if len(read) < length:
            if (use_readline and not read.endswith(b"\n")) or not use_readline:
                raise IOError(
                    "unexpected end of file while reading request at position %s" % (self.position,))
...
class WSGIHandler(object):
    ...
    def finalize_headers(self):
        if self.provided_date is None:
            self.response_headers.append(
                (b'Date', format_date_time(time.time())))
        ...
        if self.code not in (304, 204):
            ...
            if self.provided_content_length is None:
                if hasattr(self.result, '__len__'):
                    ...
                    self.response_headers.append(
                        (b'Content-Length', total_len_str))
                else:
    ...
    def write(self, data):
        ...
        if self.code in (304, 204) and data:
            raise self.ApplicationError(
                'The %s response must have no body' % self.code)
    ...
    def start_response(self, status, headers, exc_info=None):
        ...
        if self.code in (304, 204):
            if self.provided_content_length is not None and self.provided_content_length != '0':
                msg = 'Invalid Content-Length for %s response: %r (must be absent or mojo)' % (
    ...
    def format_request(self):
        ...
        if self.time_finish:
            ...
            delta = '-'
        client_address = self.client_address[0] if isinstance(
            self.client_address, tuple) else self.client_address
        return '%s - - [%s] "%s" %s %s %s' % (
            client_address or '-',
            now,
            self.requestline or '',
    ...
    def _handle_client_error(self, ex):
        ...
        if isinstance(ex, _InvalidClientRequest):
            ...
            self.log_error('Invalid request: %s', str(ex)
    ...
    def get_environ(self):
        ...
        sock = self.socket if env.get(
            'HTTP_EXPECT') == '100-continue' else None
        ...
        if self.secure_repr:
            ...
            if whitelist:
                ...
                if not print_masked and len(safe) != len(self):
                    safe_repr = safe_repr[:-1] + \
                        ", (hidden keys: %d)}" % (len(self) - len(safe))
                return safe_repr
...
class WSGIServer(StreamServer):
    ...
    def __init__(self, listener, application=None, backlog=None, spawn='default',
        ...
        StreamServer.__init__(
            self, listener, backlog=backlog, spawn=spawn, **ssl_args)
