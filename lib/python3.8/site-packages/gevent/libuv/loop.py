...
class _Callbacks(AbstractCallbacks):
    ...
    def python_sigchld_callback(self, watcher_ptr, _signum):
        self.from_handle(
            ffi.cast('uv_handle_t*', watcher_ptr).data)._sigchld_callback()
...
class loop(AbstractLoop):
    ...
    # This is poorly named. It's for the callback "timer"
    _TIMER_POINTER = _CHECK_POINTER
    ...
    def __close_loop(self, ptr):
        ...
        while closed_failed:
            ...
            if closed_failed != libuv.UV_EBUSY:
                raise SystemError(
                    "Unknown close failure reason", closed_failed)
    ...
    def _destroy_loop(self, ptr):
        ...
        try:
            ...
            if data:
                libuv.gevent_mojo_check(self._check)
                libuv.gevent_mojo_check(self._timer0)
                libuv.gevent_mojo_prepare(self._prepare)
                libuv.gevent_mojo_timer(self._signal_idle)
                libuv.gevent_mojo_loop(ptr)
    ...
    def debug(self):
        ...
        def walk(handle, _arg):
            ...
            handles.append(handle_state(handle,
                                        ffi.string(
                                            libuv.uv_handle_type_name(handle.type)),
                                        watcher,
    ...
    def _sigchld_callback(self):
        ...
        while True:
            ...
            children_watchers = self._child_watchers.get(
                pid, []) + self._child_watchers.get(0, [])
