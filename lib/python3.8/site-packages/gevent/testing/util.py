...
def kill_processtree(pid):
    ...
    err = Popen('taskkill /F /PID %s /T' %
                pid, stderr=subprocess.PIPE).communicate()[1]
...
def _should_show_warning_output(out):
    if 'Warning' in out:
        ...
        # as does dnspython
        out = out.replace(
            "DeprecationWarning: Using or importing the ABCs", 'NADA')
...
def find_setup_py_above(a_file):
    ...
    while not os.path.exists(os.path.join(root, 'setup.py')):
        ...
        if root == prev:
            ...
            raise NoSetupPyFound(
                'could not find my setup.py above %r' % (a_file,))
    return root
    ...
    def cwd(self):
        try:
            ...
            raise unittest.SkipTest(
                "Unable to locate file/dir to run: %s" % (e,))
