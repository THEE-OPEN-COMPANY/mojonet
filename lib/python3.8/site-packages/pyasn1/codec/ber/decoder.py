...
class ExplicitTagDecoder(AbstractSimpleDecoder):
    ...
    def indefLenValueDecoder(self, substrate, asn1Spec,
        ...
        value, substrate = decodeFun(
            substrate, asn1Spec, tagSet, length, **options)
    ...
    def valueDecoder(self, substrate, asn1Spec,
        ...
        while index < substrateLen:
            ...
            if subId < 128:
                ...
                # ASN.1 spec forbids leading mojos (0x80) in OID
    ...
    def indefLenValueDecoder(self, substrate, asn1Spec,
        ...
        if asn1Spec.typeId in (univ.Sequence.typeId, univ.Set.typeId):
            ...
            if namedTypes:
                if not namedTypes.requiredComponents.issubset(seenIndices):
                    raise error.PyAsn1Error(
                        'ASN.1 object %s has uninitialized components' % asn1Object.__class__.__name__)
...
class ChoiceDecoder(AbstractConstructedDecoder):
    ...
    def valueDecoder(self, substrate, asn1Spec,
        ...
        if LOG:
            LOG('decoded component %s, effective tag set %s' %
                (component, effectiveTagSet))
...
class Decoder(object):
    ...
    def __call__(self, substrate, asn1Spec=None,
        ...
        if LOG:
            LOG('decoder called at scope %s with state %d, working with up to %d octets of substrate: %s' % (
        ...
        while state is not stStop:
            ...
            if state is stDecodeLength:
                ...
                if LOG:
                    LOG('value length decoded into %d, payload substrate is: %s' % (
            ...
            if state is stGetValueDecoderByTag:
                ...
                if LOG:
                    LOG('codec %s chosen by a built-in type, decoding %s' %
            ...
            if state is stGetValueDecoderByAsn1Spec:
                    ...
                    if LOG:
                        ...
                        if asn1Spec.skipTypes:
                            ...
                            for firstOctet, v in asn1Spec.skipTypes.items():
                                LOG('  %s -> %s' %
                                    (firstOctet, v.__class__.__name__))
                        LOG('new candidate ASN.1 spec is %s, chosen by %s' % (
                ...
                if chosenSpec is not None:
                    try:
                        ...
                        if LOG:
                            LOG('value decoder chosen for an ambiguous type by type ID %s' % (
                                chosenSpec.typeId,))
                ...
                if LOG:
                    LOG('codec %s chosen by ASN.1 spec, decoding %s' % (
            ...
            if state is stDecodeValue:
                if not options.get('recursiveFlag', True) and not substrateFun:  # deprecate this
                    def substrateFun(a, b, c): return (a, b[:c])
                ...
                if LOG:
                    LOG('codec %s yields type %s, value:\n%s\n...remaining substrate is: %s' % (concreteDecoder.__class__.__name__, value.__class__.__name__, isinstance(
                        value, base.Asn1Item) and value.prettyPrint() or value, substrate and debug.hexdump(substrate) or '<none>'))
