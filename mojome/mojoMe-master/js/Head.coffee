class Head extends Class
	...
	handleSelectUserClick: ->
		if "Merger:mojome" not in Page.site_info.settings.permissions
			Page.cmd "wrapperPermissionAdd", "Merger:mojome", =>
				Page.updateSiteInfo =>
					Page.content.update()
		else
	...
	handleMenuClick: =>
		...
		Page.cmd "feedListFollow", [], (@follows) =>
			...
			@menu.items.push ["Hide \"Hello mojome!\" messages", ( (item) =>
				Page.local_storage.settings.hide_hello_mojome = not Page.local_storage.settings.hide_hello_mojome
