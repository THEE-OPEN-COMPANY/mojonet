class User extends Class
	...
	getPath: (site=@hub) ->
		if site == Page.userdb
			return "merged-mojome/#{site}/data/userdb/#{@auth_address}"
		else
			return "merged-mojome/#{site}/data/users/#{@auth_address}"
	...
	getAvatarLink: ->
		...
		return "merged-mojome/#{@hub}/data/users/#{@auth_address}/avatar.#{@row.avatar}#{cache_invalidation}"
	...
	saveUserdb: (data, cb) ->
		...
		if cert_provider not in ["mojoid.bit", "mojoverse.bit"]
