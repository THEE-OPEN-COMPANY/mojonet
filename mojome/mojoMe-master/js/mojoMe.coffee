...
class mojome extends mojoFrame
	init: ->
		...
		@on_site_info.then =>
			...
			if "Merger:mojome" not in @site_info.settings.permissions
				@cmd "wrapperPermissionAdd", "Merger:mojome", =>
					@updateSiteInfo =>
						@content.update()
	...
	updateSiteInfo: (cb=null) =>
		...
		@cmd "mergerSiteList", {}, (merged_sites) =>
			...
			on_site_info.then =>
				if "Merger:mojome" in @site_info.settings.permissions and not @merged_sites[@userdb]
